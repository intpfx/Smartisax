...
usage() {
  cat <<'USAGE'
Usage:
  tools/r2-portal-smoke-v0.portal4a.sh --url http://<r2-ip>:37601 --code <pairing-code>
...
checks = {
    "portalVersion": "0.5.3",
    "webrtc": "signaling-rtp-probe",
    "webrtcCodec": "H264",
    "browserPlayback": "mp4-video-element-fallback",
...
print("status_portal_version=0.5.3")
print("status_webrtc=signaling-rtp-probe")
...
if data.get("portalVersion") != "0.5.3":
    raise SystemExit("capabilities portalVersion mismatch")
if data.get("variant") != "v0.portal4a-webrtc-rtp-probe":
...
if preferred.get("browserPlayback") != "mp4-video-element-fallback":
...
if data.get("nativeWebRtcRuntime") is not False:
    raise SystemExit("nativeWebRtcRuntime should be false for v0.portal4a")
if data.get("dtlsSrtp") is not False:
    raise SystemExit("dtlsSrtp should be false for v0.portal4a")
...
PY
  ...
  echo "result=PORTAL_SMOKE_V0PORTAL4A_COMPLETED"
} 2>&1 | tee "$REPORT"
