...
SOURCE_SPARSE="${SOURCE_SPARSE:-${ROOT_DIR}/hard-rom/build/super-otatrust-v0.26a.2-launcher-entry-hide-v2cert-cachebump-exact-current.sparse.img}"
SOURCE_SHA256="${SOURCE_SHA256:-a96006fcd6c53b82aa3638411e01a36ce0bb92b02737aa5351fdd8827578e792}"

WORK_DIR="${WORK_DIR:-${ROOT_DIR}/hard-rom/work/v0.26b-sara-launcher-entry-hide-v2cert-cachebump}"
OUT_DIR="${ROOT_DIR}/hard-rom/build"
APK_OUT_DIR="${OUT_DIR}/apk"
SYSTEM_IMG="${SYSTEM_IMG:-${OUT_DIR}/system-otatrust-v0.26b-sara-launcher-entry-hide-v2cert-cachebump.img}"
OUT_SPARSE="${OUT_SPARSE:-${OUT_DIR}/super-otatrust-v0.26b-sara-launcher-entry-hide-v2cert-cachebump-exact-current.sparse.img}"
MANIFEST="${MANIFEST:-${OUT_DIR}/super-otatrust-v0.26b-sara-launcher-entry-hide-v2cert-cachebump-exact-current.SHA256SUMS.txt}"
...
SARA_RAW_APK="${APK_OUT_DIR}/com.smartisanos.sara-launcher-hidden.apk"
SARA_APK="${SARA_APK:-${APK_OUT_DIR}/com.smartisanos.sara-launcher-hidden-v2cert.apk}"
APK_MANIFEST="${APK_OUT_DIR}/launcher-entry-hide-apk-manifest-v0.26b.tsv"
...
IMAGE_TAG="${IMAGE_TAG:-v026b}"
SPARSE_VARIANT="${SPARSE_VARIANT:-otatrust-v0.26b-sara-launcher-entry-hide-v2cert-cachebump-exact-current}"
PURPOSE="${PURPOSE:-Manifest-only launcher entry hide for Sara/VoiceAssistant on top of live-verified v0.26a.2}"
PACKAGE_DIR_MTIME_HEX="${PACKAGE_DIR_MTIME_HEX:-0x6a33ebd0}"
PACKAGE_DIR_MTIME_NOTE="${PACKAGE_DIR_MTIME_NOTE:-2026-06-18 21:00:00 +0800; forces PackageCacher to ignore stale VoiceAssistant directory parse cache on first boot}"
...
Build the v0.26b Sara/VoiceAssistant launcher-entry-hide ROM candidate on top
of the live-verified v0.26a.2 sparse image. This removes only the desktop
LAUNCHER category from:

  - VoiceAssistant / com.smartisanos.sara.bubble.SettingActivity
...
Environment:
  REBUILD_APKS=0        reuse existing Sara launcher-hidden APK outputs
USAGE
}
...
if [ "$REBUILD_APKS" = "1" ]; then
  echo "Building v0.26b Sara launcher-entry-hide APK candidate..."
  "$APK_BUILDER" --variant v0.26b >/dev/null
  need_file "$SARA_RAW_APK"
  echo "Copying stock v2 signing block into Sara launcher-hidden APK..."
  "$V2_PRESERVER" --stock "$SARA_STOCK_APK" --edited "$SARA_RAW_APK" --out "$SARA_APK" >/dev/null
fi

need_file "$SARA_APK"
need_file "$APK_MANIFEST"

echo "Extracting system_b from v0.26a.2 sparse super..."
...
echo "Replacing Sara launcher-entry-hide APK in system_b..."
: > "${WORK_DIR}/replacements.tsv"
replace_file_in_image "$SYSTEM_IMG" "$SARA_APK" \
  "/system/priv-app/VoiceAssistant/VoiceAssistant.apk" \
  "system-voiceassistant-launcher-hidden" >> "${WORK_DIR}/replacements.tsv"

echo "Bumping VoiceAssistant package directory mtime for PackageCacher invalidation..."
: > "${WORK_DIR}/package-dir-mtime-bumps.tsv"
bump_package_dir_time "$SYSTEM_IMG" "/system/priv-app/VoiceAssistant" \
  "system-voiceassistant-dir" >> "${WORK_DIR}/package-dir-mtime-bumps.tsv"

echo "Checking modified system_b..."
fsck_image "$SYSTEM_IMG"

echo "Verifying replaced APK after fsck..."
dump_and_compare "$SYSTEM_IMG" "$SARA_APK" \
  "/system/priv-app/VoiceAssistant/VoiceAssistant.apk" "system-voiceassistant-launcher-hidden"
...
sara_hash="$(sha256_one "$SARA_APK")"
sara_raw_hash="$(sha256_one "$SARA_RAW_APK")"
stock_hash="$(sha256_one "$SARA_STOCK_APK")"

{
  ...
  echo "sara_stock_apk=${SARA_STOCK_APK}"
  echo "sara_stock_sha256=${stock_hash}"
  echo "sara_launcher_hidden_raw_apk=${SARA_RAW_APK}"
  echo "sara_launcher_hidden_raw_sha256=${sara_raw_hash}"
  echo "sara_launcher_hidden_v2cert_apk=${SARA_APK}"
  echo "sara_launcher_hidden_v2cert_sha256=${sara_hash}"
  ...
  shasum -a 256 "$OUT_SPARSE" "$SYSTEM_IMG" "$SOURCE_SPARSE" \
    "$SARA_STOCK_APK" "$SARA_RAW_APK" "$SARA_APK"
} > "$MANIFEST"
