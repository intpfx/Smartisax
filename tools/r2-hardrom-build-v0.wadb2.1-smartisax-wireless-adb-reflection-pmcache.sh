...
SOURCE_VARIANT="v0.wadb2-smartisax-wireless-adb-current-wifi"
SOURCE_SPARSE="${SOURCE_SPARSE:-${ROOT_DIR}/hard-rom/build/super-otatrust-v0.wadb2-smartisax-wireless-adb-current-wifi.sparse.img}"
SOURCE_SPARSE_SHA256="a542b056b356112d8a5e8a5cc2ba90103d07c3f72f82ece6d9ff028cd676144a"
SOURCE_SYSTEM_B_SHA256="e82b9259543d169a473992635ec5c29081612d2dba43a49b0379e439daad6ca8"
...
SERVICES_JAR_CANDIDATE="${SERVICES_JAR_CANDIDATE:-${ROOT_DIR}/hard-rom/build/framework/services-wadb2-privapp-cache-adb.jar}"
...
PACKAGE_DIR_MTIME_HEX="${PACKAGE_DIR_MTIME_HEX:-0x6a39d930}"
PACKAGE_DIR_MTIME_NOTE="${PACKAGE_DIR_MTIME_NOTE:-2026-06-22 21:30:00 +0800; invalidates Smartisax package scan/cache after reflection and priv-app cache-bypass repair}"

PURPOSE="Repair Smartisax wireless ADB after v0.wadb2: use declared reflection for IAdbManager hidden methods and extend PackageManager cache bypass to /system/priv-app/SmartisaxShell."
RESULT_NAME="PASS_BUILD_V0WADB21_SMARTISAX_WIRELESS_ADB_REFLECTION_PMCACHE"

usage() {
  cat <<'USAGE'
Usage:
  tools/r2-hardrom-build-v0.wadb2.1-smartisax-wireless-adb-reflection-pmcache.sh

Builds a system_b-only candidate on top of live-tested v0.wadb2. It updates the
Smartisax priv-app APK in place, replaces /system/framework/services.jar with a
v0.wadb2.1 candidate that also bypasses PackageManager cache for the Smartisax
priv-app path, rebuilds system_b AVB/FEC roots=2, and patches that system_b
image into the v0.wadb2 sparse super. It does not touch a live device.
USAGE
}
...
unzip -t "$SERVICES_JAR_CANDIDATE" >/dev/null
...

{
  ...
  replace_services_jar "$SYSTEM_B_IMG"
  {
    ...
    set_dir_time "$SYSTEM_B_IMG" "/system/framework" "system-framework-parent"
  ...
  verify_image_file_hash "$SYSTEM_B_IMG" "$SERVICES_JAR_PATH" "$SERVICES_JAR_CANDIDATE_SHA256" "services-jar-after-fsck.jar"
  ...
  system_hash="$(sha256_one "$SYSTEM_B_IMG")"
  {
    ...
    echo
    shasum -a 256 "$SYSTEM_B_IMG" "$SMARTISAX_APK" "$PRIVAPP_XML" "$SERVICES_JAR_CANDIDATE" "$SOURCE_SPARSE"
  } > "$SYSTEM_MANIFEST"
  ...
  sparse_hash="$(sha256_one "$OUT_SPARSE")"
  {
    ...
    echo "built_at=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
    echo
    shasum -a 256 "$OUT_SPARSE" "$SYSTEM_B_IMG" "$SMARTISAX_APK" "$PRIVAPP_XML" "$SERVICES_JAR_CANDIDATE" "$SOURCE_SPARSE"
  } > "$MANIFEST"
