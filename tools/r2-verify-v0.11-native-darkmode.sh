...
EXPECTED_SUPER="${ROOT_DIR}/hard-rom/build/super-otatrust-v0.11-native-darkmode-exact-current.sparse.img"
EXPECTED_SYSTEM_IMG="${ROOT_DIR}/hard-rom/build/system-otatrust-v0.11-native-darkmode.img"
EXPECTED_SYSTEM_EXT_IMG="${ROOT_DIR}/hard-rom/build/system_ext-otatrust-v0.11-native-darkmode.img"
...
Usage:
  ...
  tools/r2-verify-v0.11-native-darkmode.sh --read-only

--offline-image verifies the generated v0.11 sparse super:
  - v0.11 APK semantic verifier still passes
...
run_offline_image() {
  ...
  report="${INSPECT_DIR}/verify-v0.11-native-darkmode-offline-image-${timestamp}.txt"
  ...
  {
    echo "# v0.11 native dark-mode ROM offline verification"
    ...
    verify_held_path "$EXPECTED_SYSTEM_IMG" "/system/priv-app/SettingsSmartisan/.SettingsSmartisan.apk.smartisax-v011-stock-held"
    echo
    ...
    echo
    echo "PASS: v0.11 native dark-mode offline image verification"
  } | tee "$report"
...
run_read_only_device() {
  ...
  local pull_dir
  timestamp="$(date +%Y%m%d-%H%M%S)"
  report="${INSPECT_DIR}/verify-v0.11-native-darkmode-device-${timestamp}.txt"
  pull_dir="${INSPECT_DIR}/device-${timestamp}"
  mkdir -p "$pull_dir"

  {
    echo "# v0.11 native dark-mode device verification"
    echo "timestamp=${timestamp}"
    echo "serial=${SERIAL}"
    echo "expected_super=${EXPECTED_SUPER}"
  ...
  {
    echo
    echo "PASS: v0.11 native dark-mode device read-only verification"
    echo "Report: ${report}"
  } | tee -a "$report"
}
