...
verify_apk_semantics() {
  ...
  if [ "$EXPECTED_WEBRTC_SESSION_CONTROL" = "1" ]; then
    ...
    grep -R -q 'H264-preferred-browser' "${decode_dir}/smali" || die "H264 preferred status marker missing"
    grep -R -q 'selectedCodec' "${decode_dir}/smali" || die "selectedCodec answer marker missing"
    grep -q 'preferVideoCodec(transceiver, "H264")' "${decode_dir}/assets/portal/index.html" \
  ...
  if [ "$EXPECTED_WEBRTC_INPUT_CHANNEL" = "1" ]; then
    ...
    grep -q 'webRtcInputChannel.send' "${decode_dir}/assets/portal/index.html" \
  ...
  if [ "$EXPECTED_WEBRTC_CAPTURE_PROBE" = "1" ]; then
    ...
    grep -q 'max="60"' "${decode_dir}/assets/portal/index.html" \
      || die "Portal asset fps controls do not expose 60fps"
...
read_only_device() {
  ...
  local report="${INSPECT_DIR}/verify-${VARIANT}-device-read-only-$(date '+%Y%m%d-%H%M%S').txt"
  {
    ...
    echo
    echo "## package"
    adb -s "$SERIAL" shell 'pm path com.smartisax.browser; dumpsys package com.smartisax.browser | grep -E "versionCode=|versionName=|codePath=|resourcePath=|nativeLibraryDir=|primaryCpuAbi=|READ_FRAME_BUFFER|CAPTURE_VIDEO_OUTPUT|INJECT_EVENTS|MANAGE_DEBUGGING|WRITE_SECURE_SETTINGS|MANAGE_MEDIA_PROJECTION" | head -n 90' | tr -d '\r'
