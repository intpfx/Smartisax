...
SOURCE_VARIANT="v0.pm0-services-jar-noop"
SOURCE_SPARSE="${SOURCE_SPARSE:-${ROOT_DIR}/hard-rom/build/super-otatrust-v0.pm0-services-jar-noop.sparse.img}"
SOURCE_SPARSE_SHA256="4834d9d233e7243f61211b81b73e15fb3f293d45d80fcecbc7612bad6c4cf1c7"
SOURCE_SYSTEM_B_SHA256="e6341016f5f453f5734916c88fa3efaa51c937f9533f58b9e36cf36a3a43440e"
SERVICES_JAR_CANDIDATE="${SERVICES_JAR_CANDIDATE:-${ROOT_DIR}/hard-rom/build/framework/services-pm1-cache-allowlist.jar}"
...
PURPOSE="Install the v0.pm1 Smartisax PackageManager package-parser cache allowlist policy into services.jar on top of live-proven v0.pm0."
RESULT_NAME="PASS_BUILD_V0PM1_PMS_CACHE_ALLOWLIST_SYSTEM_IMAGE"
...
Builds only a system_b image candidate for the first narrow PackageManager
behavior policy. This script does not build super, flash, reboot, or touch a
live device.

The image-level gate:
  - extracts system_b from the live-proven v0.pm0 sparse super
  - replaces only /system/framework/services.jar
  - keeps public services preopt artifacts absent
...
replace_services_jar() {
  ...
  assert_unique_blocks_for_delete "$image" "$SERVICES_JAR_PATH" "services-jar-pm0"
...

{
  ...
  echo "## extract system_b from v0.pm0 sparse"
  "$SPARSE_TOOL" \
  ...
  changed_entries="$(awk -F= '$1 == "changed_entries" {print $2}' "${WORK_DIR}/services-jar-delta.txt")"
  {
    ...
    echo "purpose=${PURPOSE}"
    echo "flash_gate=system_b image only; no super built; explicit user confirmation required before any live flash"
    echo "image_mode=system_b_from_v0pm0_sparse"
    echo "source_variant=${SOURCE_VARIANT}"
    echo "source_sparse=${SOURCE_SPARSE}"
    echo "source_sparse_sha256=${SOURCE_SPARSE_SHA256}"
    echo "source_system_b_extent=${SYSTEM_B_EXTENT}"
