...
usage() {
  cat <<'USAGE'
Usage:
  tools/r2-verify-v0.portal4a-webrtc-rtp-probe.sh --offline-image
  tools/r2-verify-v0.portal4a-webrtc-rtp-probe.sh --read-only
...
verify_apk_semantics() {
  ...
  grep -q "package: name='com.smartisax.browser' versionCode='13' versionName='0.5.3'" "${WORK_DIR}/smartisax-aapt-badging.txt" \
  ...
  grep -R -q 'signaling-rtp-probe' "${decode_dir}/smali" || die "portal WebRTC/RTP probe marker missing"
  grep -R -q 'mp4-video-element-fallback' "${decode_dir}/smali" || die "portal MP4 fallback marker missing"
  grep -R -q 'v0.portal4a-webrtc-rtp-probe' "${decode_dir}/smali" || die "portal4a variant marker missing"
...
run_offline_image() {
  ...
  local report="${INSPECT_DIR}/verify-${VARIANT}-offline-image-$(date '+%Y%m%d-%H%M%S').txt"
  {
    ...
    echo "result=PASS_OFFLINE_IMAGE_V0PORTAL4A_WEBRTC_RTP_PROBE"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
...
printf "init.svc.bootanim=%s\n" "$(getprop init.svc.bootanim)"'
    ...
    grep -q 'versionCode=13 ' <<<"$package_dump" || die "Smartisax PackageManager did not parse versionCode=13"
    ...
    echo "result=PASS_READ_ONLY_V0PORTAL4A_WEBRTC_RTP_PROBE"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
