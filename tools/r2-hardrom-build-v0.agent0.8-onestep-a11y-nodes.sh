...
SOURCE_VARIANT="v0.agent0.7-window-preflight"
...
SOURCE_SPARSE="${SOURCE_SPARSE:-${ROOT_DIR}/hard-rom/build/super-otatrust-v0.agent0.7-window-preflight.sparse.img}"
SOURCE_SPARSE_SHA256="${SOURCE_SPARSE_SHA256:-d16518056abea641cf51e8d944eb517a00dfdbd3d4ba7ef44a5cbad30400c7cc}"
...
PACKAGE_DIR_MTIME_HEX="${PACKAGE_DIR_MTIME_HEX:-0x6a4ad3e0}"
PACKAGE_DIR_MTIME_NOTE="${PACKAGE_DIR_MTIME_NOTE:-2026-07-06 06:00:00 +0800; invalidates Smartisax/Sidebar package scan cache after Agent v0.7.8 and One Step app-node APK update}"

PURPOSE="Agent v0.8 repair on top of live v0.agent0.7: add One Step enter visible-state recovery in SmartisaxShell and patch Sidebar dynamic top app strip with Agent-friendly Accessibility nodes and ACTION_CLICK AppItem.openUI binding."
RESULT_NAME="PASS_BUILD_V0AGENT08_ONESTEP_A11Y_NODES"

usage() {
  cat <<'USAGE'
Usage:
  tools/r2-hardrom-build-v0.agent0.8-onestep-a11y-nodes.sh

Build the v0.agent0.8 Agent/One Step candidate on top of live-proven
v0.agent0.7-window-preflight. This updates SmartisaxShell and replaces only:

  /system/priv-app/Sidebar/Sidebar.apk

with a classes.dex-only Sidebar patch that exposes dynamic top app strip items
as clickable Accessibility nodes. The script rebuilds system_b AVB/FEC and
patches it into the v0.agent0.7 sparse super. It does not flash, reboot, erase
misc, or touch /data.
USAGE
}
...

{
  ...
  echo "## build Sidebar Agent-friendly One Step APK"
