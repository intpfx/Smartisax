...
usage() {
  cat <<'USAGE'
Usage:
  tools/r2-verify-v0.portal2.2-smartisax-remote-screen-control-bufferfix.sh --offline-image
  tools/r2-verify-v0.portal2.2-smartisax-remote-screen-control-bufferfix.sh --read-only
...
--offline-image verifies the candidate without touching a device:
  ...
  - Smartisax contains the privileged SurfaceControl PNG screenshot path,
    ScreenshotGraphicBuffer-to-Bitmap conversion, and InputManager tap/swipe
    injection paths for the browser-control MVP
...
verify_apk_semantics() {
  ...
  grep -q "package: name='com.smartisax.browser' versionCode='9' versionName='0.4.2'" "${WORK_DIR}/smartisax-aapt-badging.txt" \
  ...
  grep -R -q 'getGraphicBuffer' "${decode_dir}/smali/com/smartisax/browser/DevicePortalService"*.smali || die "ScreenshotGraphicBuffer getGraphicBuffer path missing"
  grep -R -q 'wrapHardwareBuffer' "${decode_dir}/smali/com/smartisax/browser/DevicePortalService"*.smali || die "ScreenshotGraphicBuffer wrapHardwareBuffer path missing"
  grep -R -q 'surfacecontrol_screenshot_buffer_convert_failed' "${decode_dir}/smali/com/smartisax/browser/DevicePortalService"*.smali || die "ScreenshotGraphicBuffer conversion diagnostic missing"
...
run_offline_image() {
  ...
  local report="${INSPECT_DIR}/verify-${VARIANT}-offline-image-$(date '+%Y%m%d-%H%M%S').txt"
  {
    ...
    echo "result=PASS_OFFLINE_IMAGE_V0PORTAL22_SMARTISAX_REMOTE_SCREEN_CONTROL_BUFFERFIX"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
...
printf "init.svc.bootanim=%s\n" "$(getprop init.svc.bootanim)"'
    ...
    echo "result=PASS_READ_ONLY_V0PORTAL22_SMARTISAX_REMOTE_SCREEN_CONTROL_BUFFERFIX"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
