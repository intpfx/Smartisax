...
SIDEBAR_APK_BUILDER="${SIDEBAR_APK_BUILDER:-${ROOT_DIR}/tools/r2-build-sidebar-font-ocr-disabled-apk.sh}"
...
SIDEBAR_APK="${SIDEBAR_APK:-${ROOT_DIR}/hard-rom/build/apk/com.smartisanos.sidebar-font-ocr-disabled-v2cert.apk}"
SIDEBAR_APK_SHA256="b0a7c04626c41bf3e36957f2fb56cfee9af3b35c29a3ea9a830a0803990e9e7d"
...
PURPOSE="Retire Sidebar/One Step font OCR on top of live-stable v0.37b, preserving TextBoom, M150 WebView, Smartisax shell, v0.29 Sidebar topbar blank-slot behavior, and system_b AVB/FEC."
...

{
  ...
  replace_file_in_image "$SYSTEM_B_IMG" "$SIDEBAR_APK" "$SIDEBAR_PATH" \
    "sidebar-font-ocr-disabled" >> "${WORK_DIR}/replacements.tsv"
  ...
  python3 "$AVBTOOL" info_image --image "$SYSTEM_B_IMG" > "${WORK_DIR}/system-b-v038-avb-info.txt"
  grep -q "FEC num roots:         2" "${WORK_DIR}/system-b-v038-avb-info.txt" || die "system_b lost FEC roots"
...

  {
    ...
    echo "sidebar_font_ocr=disabled"
    echo "sidebar_boom_font_activity=disabled"
    echo "sidebar_action_boom_font=removed"
    echo "sidebar_font_button_layout=gone"
  ...
  echo "result=PASS_BUILD_V038_SIDEBAR_FONT_OCR_DISABLED"
} 2>&1 | tee "$REPORT"
