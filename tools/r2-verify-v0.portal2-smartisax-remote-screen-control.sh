...
usage() {
  cat <<'USAGE'
Usage:
  tools/r2-verify-v0.portal2-smartisax-remote-screen-control.sh --offline-image
  tools/r2-verify-v0.portal2-smartisax-remote-screen-control.sh --read-only
...
--offline-image verifies the candidate without touching a device:
  ...
  - Smartisax contains the root screencap PNG stream and root input tap/swipe
    command paths for the browser-control MVP
...
verify_apk_semantics() {
  ...
  grep -q "package: name='com.smartisax.browser' versionCode='7' versionName='0.4.0'" "${WORK_DIR}/smartisax-aapt-badging.txt" \
  ...
  grep -R -q 'root-screencap-png' "${decode_dir}/smali/com/smartisax/browser/DevicePortalService"*.smali || die "portal screen backend marker missing"
  grep -R -q 'root-input-command' "${decode_dir}/smali/com/smartisax/browser/DevicePortalService"*.smali || die "portal input backend marker missing"
  grep -R -q 'screencap -p' "${decode_dir}/smali/com/smartisax/browser/DevicePortalService"*.smali || die "root screencap path missing"
  grep -R -q 'input tap' "${decode_dir}/smali/com/smartisax/browser/DevicePortalService"*.smali || die "root input tap path missing"
  grep -R -q 'input swipe' "${decode_dir}/smali/com/smartisax/browser/DevicePortalService"*.smali || die "root input swipe path missing"
...
run_offline_image() {
  ...
  local report="${INSPECT_DIR}/verify-${VARIANT}-offline-image-$(date '+%Y%m%d-%H%M%S').txt"
  {
    ...
    echo "result=PASS_OFFLINE_IMAGE_V0PORTAL2_SMARTISAX_REMOTE_SCREEN_CONTROL"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
...
printf "init.svc.bootanim=%s\n" "$(getprop init.svc.bootanim)"'
    ...
    grep -E "codePath=|resourcePath=|versionCode=|pkgFlags=|privateFlags=|android.permission.(MANAGE_DEBUGGING|WRITE_SECURE_SETTINGS|ACCESS_WIFI_STATE):" <<<"$package_dump" || true
    grep -q 'versionCode=7 ' <<<"$package_dump" || die "Smartisax PackageManager did not parse versionCode=7"
    ...
    echo "result=PASS_READ_ONLY_V0PORTAL2_SMARTISAX_REMOTE_SCREEN_CONTROL"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
