...
WORK_DIR="${WORK_DIR:-${ROOT_DIR}/hard-rom/work/v0.11.1-native-darkmode-settings-row}"
...
SYSTEM_IMG="${OUT_DIR}/system-otatrust-v0.11.1-native-darkmode-settings-row.img"
SYSTEM_EXT_IMG="${OUT_DIR}/system_ext-otatrust-v0.11.1-native-darkmode-settings-row.img"
OUT_SPARSE="${OUT_DIR}/super-otatrust-v0.11.1-native-darkmode-settings-row-exact-current.sparse.img"
MANIFEST="${OUT_DIR}/super-otatrust-v0.11.1-native-darkmode-settings-row-exact-current.SHA256SUMS.txt"
...
Build the v0.11.1 combined native dark-mode behavior ROM image on the
live-verified v0.24 line. This is the v0.11 follow-up that keeps the same
UiMode/SystemUI behavior and moves the SettingsSmartisan brightness-row patch
after the Darwin :cond_5 branch so the row is reachable on R2.

  - replaces SettingsSmartisan.apk in system_b with the v0.11.1 dark-mode UI and
    quick-widget editor patch using the shared_blocks-safe held-stock-inode path
  - replaces SmartisanSystemUI.apk in system_ext_b with a same-size v0.11.1
...
Environment:
  ...
  REBUILD_APKS=0        reuse existing v0.11.1 APK outputs
USAGE
}
...
replace_file_in_image() {
  ...
  dir="$(dirname "$dst")"
  base="$(basename "$dst")"
  temp_path="${dir}/.${base}.smartisax-v0111-tmp"
  held_path="${dir}/.${base}.smartisax-v0111-stock-held"
...
if [ "$REBUILD_APKS" = "1" ]; then
  echo "Building v0.11.1 native dark-mode APK candidates..."
...
echo "Verifying v0.11.1 APK semantics..."
...
"$SPARSE_TOOL" \
  ...
  --variant "otatrust-v0.11.1-native-darkmode-settings-row-exact-current"
...
{
  echo "variant=otatrust-v0.11.1-native-darkmode-settings-row-exact-current"
  echo "purpose=Combined native dark-mode SettingsSmartisan UI/editor patch with reachable Darwin brightness row and SmartisanSystemUI toggleDarkMode tile patch"
