...
usage() {
  cat <<'USAGE'
Usage:
  tools/r2-verify-v0.portal3a-webrtc-capability-probe.sh --offline-image
  tools/r2-verify-v0.portal3a-webrtc-capability-probe.sh --read-only
...
verify_apk_semantics() {
  ...
  grep -q "package: name='com.smartisax.browser' versionCode='10' versionName='0.5.0'" "${WORK_DIR}/smartisax-aapt-badging.txt" \
  ...
  grep -R -q 'capability-probe' "${decode_dir}/smali" || die "portal WebRTC capability-probe marker missing"
  grep -R -q 'v0.portal3a-webrtc-capability-probe' "${decode_dir}/smali" || die "portal3a variant marker missing"
...
run_offline_image() {
  ...
  local report="${INSPECT_DIR}/verify-${VARIANT}-offline-image-$(date '+%Y%m%d-%H%M%S').txt"
  {
    ...
    echo "result=PASS_OFFLINE_IMAGE_V0PORTAL3A_WEBRTC_CAPABILITY_PROBE"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
...
printf "init.svc.bootanim=%s\n" "$(getprop init.svc.bootanim)"'
    ...
    grep -q 'versionCode=10 ' <<<"$package_dump" || die "Smartisax PackageManager did not parse versionCode=10"
    ...
    echo "result=PASS_READ_ONLY_V0PORTAL3A_WEBRTC_CAPABILITY_PROBE"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
