...
usage() {
  cat <<'USAGE'
Usage:
  tools/r2-verify-v0.portal4c-session-hardening.sh --offline-image
  tools/r2-verify-v0.portal4c-session-hardening.sh --read-only
...
verify_apk_semantics() {
  ...
  grep -q "package: name='com.smartisax.browser' versionCode='15' versionName='0.5.5'" "${WORK_DIR}/smartisax-aapt-badging.txt" \
  ...
  grep -R -q '/api/rtp/h264' "${decode_dir}/smali" || die "portal /api/rtp/h264 path missing from dex"
  grep -R -q '/api/webrtc/offer' "${decode_dir}/smali" || die "portal /api/webrtc/offer path missing from dex"
  grep -R -q 'signaling-rtp-probe-mp4-live-session-hardening' "${decode_dir}/smali" || die "portal WebRTC/RTP probe marker missing"
  grep -R -q 'mp4-live-loop-fallback' "${decode_dir}/smali" || die "portal MP4 live fallback marker missing"
  grep -R -q 'v0.portal4c-session-hardening' "${decode_dir}/smali" || die "portal4c variant marker missing"
  grep -R -q 'bearer-token-pair-code-rotation' "${decode_dir}/smali" || die "portal access-control marker missing"
  grep -R -q 'rotates-after-success' "${decode_dir}/smali" || die "portal pairing rotation marker missing"
  grep -R -q 'pairing_temporarily_locked' "${decode_dir}/smali" || die "portal bad-pair lockout marker missing"
  grep -R -q 'constantTimeEquals' "${decode_dir}/smali" || die "portal constant-time token comparison missing"
  grep -R -q 'Content-Security-Policy' "${decode_dir}/smali" || die "portal CSP header missing"
  grep -R -q 'Referrer-Policy' "${decode_dir}/smali" || die "portal referrer policy header missing"
  ...
  grep -R -q 'application/x-smartisax-rtp-dump' "${decode_dir}/smali" || die "RTP dump MIME path missing"
  grep -R -q 'rtp_packetizer_no_nals' "${decode_dir}/smali" || die "RTP packetizer path missing"
  ...
  grep -q 'Start Live' "$portal_asset" || die "Portal asset missing Live playback control"
  grep -q 'liveMetrics' "$portal_asset" || die "Portal asset missing Live metrics"
  grep -q 'sessionState' "$portal_asset" || die "Portal asset missing session status"
  grep -q 'Forget Session' "$portal_asset" || die "Portal asset missing local-session clear control"
  grep -q 'localStorage.smartisaxToken' "$portal_asset" || die "Portal asset missing local token storage path"
  grep -q 'pair expired' "$portal_asset" || die "Portal asset missing expired-session UX"
  grep -q 'autoplay.*live' "$portal_asset" || die "Portal asset missing Live autoplay path"
  ...
  grep -q 'WebRTC Offer' "$portal_asset" || die "Portal asset missing WebRTC offer control"
  grep -q 'Probe RTP' "$portal_asset" || die "Portal asset missing RTP probe control"
  grep -q '/api/webrtc/offer' "$portal_asset" || die "Portal asset missing WebRTC offer endpoint"
  grep -q '/api/rtp/h264' "$portal_asset" || die "Portal asset missing RTP endpoint"
  grep -q 'RTCPeerConnection' "$portal_asset" || die "Portal asset missing browser WebRTC offer path"
  grep -q 'parseRtpDump' "$portal_asset" || die "Portal asset missing RTP dump parser"
...
run_offline_image() {
  ...
  local report="${INSPECT_DIR}/verify-${VARIANT}-offline-image-$(date '+%Y%m%d-%H%M%S').txt"
  {
    ...
    echo "result=PASS_OFFLINE_IMAGE_V0PORTAL4C_SESSION_HARDENING"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
...
printf "init.svc.bootanim=%s\n" "$(getprop init.svc.bootanim)"'
    ...
    grep -q 'versionCode=15 ' <<<"$package_dump" || die "Smartisax PackageManager did not parse versionCode=15"
    ...
    echo "result=PASS_READ_ONLY_V0PORTAL4C_SESSION_HARDENING"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
