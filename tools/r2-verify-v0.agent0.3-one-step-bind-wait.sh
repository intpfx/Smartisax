...
export EXPECTED_VERSION_CODE="54"
export EXPECTED_VERSION_NAME="0.7.3"
export EXPECTED_NATIVE_SYSTEM_WEBRTC_LIBS="1"
export EXPECTED_PORTAL_VARIANT_MARKER="v0.agent0.3-one-step-bind-wait"
...
export OFFLINE_RESULT_NAME="PASS_OFFLINE_IMAGE_V0AGENT03_ONE_STEP_BIND_WAIT"
export READ_ONLY_RESULT_NAME="PASS_READ_ONLY_V0AGENT03_ONE_STEP_BIND_WAIT"
...
{
  echo
  echo "## agent0.3 extra offline checks"
  ...
  grep -R -q 'v0.agent0.3-one-step-bind-wait' "$decode_dir" || { echo "error: agent0.3 variant marker missing" >&2; exit 1; }
  ...
  echo "agent03_extra_offline_checks=ok"
} 2>&1 | tee -a "$report"
