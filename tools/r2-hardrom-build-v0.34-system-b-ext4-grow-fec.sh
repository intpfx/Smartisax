...
CANDIDATE_RAW="${WORK_DIR}/candidate-v034-fec-super.raw.img"
CANDIDATE_EXTRACT_DIR="${WORK_DIR}/candidate-v034-fec-slot1"
OUT_RAW_FOR_LPDUMP="${WORK_DIR}/candidate-v034-fec-super.raw-for-lpdump.img"
...
SYSTEM_B_FEC_OVERHEAD_BYTES=$((SYSTEM_B_PARTITION_SIZE - SYSTEM_B_FEC_EXT4_SIZE))
...
PURPOSE="Offline ext4 capacity gate on top of live-verified v0.33. It keeps partition layout at the v0.33 grown system_b size, expands the system_b ext4 filesystem to the maximum FEC-preserving AVB hashtree data size, rebuilds system_b hashtree footer with Android FEC roots=2, and preserves all package/file payloads."
...
This FEC-preserving gate keeps the stock-style system_b hashtree/FEC footer
shape while growing ext4. It does not flash, reboot, erase misc, write
settings, clear package cache, or mutate /data.
USAGE
}
...
verify_candidate_extract() {
  ...
  printf 'system_b\tgrown_fec_sha256=%s\n' "$candidate_hash"
}
...

{
  ...
  echo "flash_gate=offline-only; FEC-preserving candidate requires explicit promotion before live flashing"
  echo "fec_tool=${FEC}"
  echo
  ...
  truncate -s "$SYSTEM_B_FEC_EXT4_SIZE" "$SYSTEM_B_IMG"
  "$RESIZE2FS" -f "$SYSTEM_B_IMG" "$SYSTEM_B_FEC_BLOCKS" >/dev/null
  fsck_rw "$SYSTEM_B_IMG"
  ...
  [ "$grown_blocks" = "$SYSTEM_B_FEC_BLOCKS" ] || die "grown block count mismatch: ${grown_blocks} != ${SYSTEM_B_FEC_BLOCKS}"
  ...
  echo "system_b_ext4_growth_bytes=$((SYSTEM_B_FEC_EXT4_SIZE - SYSTEM_B_SOURCE_EXT4_SIZE))"
  echo "system_b_fec_avb_overhead_bytes=${SYSTEM_B_FEC_OVERHEAD_BYTES}"
  ...
  grep -q "Original image size:      ${SYSTEM_B_FEC_EXT4_SIZE} bytes" "${WORK_DIR}/candidate-system-b-avb-info.txt" || die "candidate original image size mismatch"
  grep -q "FEC num roots:         2" "${WORK_DIR}/candidate-system-b-avb-info.txt" || die "candidate lost FEC roots"
  grep -q "FEC offset:            [1-9]" "${WORK_DIR}/candidate-system-b-avb-info.txt" || die "candidate FEC offset was not populated"
...

  {
    ...
    echo "system_b_fec_ext4_size=${SYSTEM_B_FEC_EXT4_SIZE}"
    echo "system_b_ext4_growth_bytes=$((SYSTEM_B_FEC_EXT4_SIZE - SYSTEM_B_SOURCE_EXT4_SIZE))"
    echo "system_b_fec_avb_overhead_bytes=${SYSTEM_B_FEC_OVERHEAD_BYTES}"
    echo "source_system_b_block_count=${source_blocks}"
    echo "source_system_b_free_blocks=${source_free_blocks}"
    echo "grown_system_b_block_count=${grown_blocks}"
    echo "grown_system_b_free_blocks=${grown_free_blocks}"
    echo "fec_status=generated_roots_2"
    echo "fec_tool=${FEC}"
