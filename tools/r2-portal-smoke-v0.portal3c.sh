...
usage() {
  cat <<'USAGE'
Usage:
  tools/r2-portal-smoke-v0.portal3c.sh --url http://<r2-ip>:37601 --code <pairing-code>
...
checks = {
    "portalVersion": "0.5.2",
    "webrtc": "h264-mp4-browser-playback",
    "webrtcCodec": "H264",
    "browserPlayback": "mp4-video-element",
...
print("status_portal_version=0.5.2")
print("status_webrtc=h264-mp4-browser-playback")
...
if data.get("portalVersion") != "0.5.2":
    raise SystemExit("capabilities portalVersion mismatch")
if data.get("variant") != "v0.portal3c-h264-webcodecs-playback":
...
if preferred.get("browserPlayback") != "mp4-video-element":
...
PY
  ...
  echo "result=PORTAL_SMOKE_V0PORTAL3C_COMPLETED"
} 2>&1 | tee "$REPORT"
