#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
COVERAGE_TSV="${COVERAGE_TSV:-${ROOT_DIR}/reverse/smartisan-8.5.3-rom-static/manifest/language-full-prune-coverage-audit.tsv}"
POLICY="${ROOT_DIR}/tools/r2-verify-apk-locale-policy.py"
INSPECT_DIR="${ROOT_DIR}/hard-rom/inspect/apk-only-locale-prune-candidates"

usage() {
  cat <<'USAGE'
Usage:
  tools/r2-verify-apk-only-locale-prune-candidates.sh

Read-only verifier for APK-only language hard-prune candidates listed in
language-full-prune-coverage-audit.tsv. It verifies each apk_only_* row:

  - output APK sha256 matches the coverage manifest
  - ZIP integrity is valid
  - classes.dex and AndroidManifest.xml stay byte-identical to stock when present
  - resources.arsc changes
  - resources.arsc is stored, matching stock system APK resource-table shape
  - binary resources.arsc locale policy contains only English/Chinese chunks
  - signature boundary is the expected resources.arsc digest error

This does not build ROM images, flash, reboot, or touch /data.
USAGE
}

need_file() {
  [ -f "$1" ] || {
    echo "error: missing file: $1" >&2
    exit 1
  }
}

entry_exists() {
  local apk="$1"
  local entry="$2"
  unzip -Z1 "$apk" | awk -v entry="$entry" '$0 == entry { found = 1 } END { exit found ? 0 : 1 }'
}

entry_hash() {
  local apk="$1"
  local entry="$2"
  unzip -p "$apk" "$entry" | shasum -a 256 | awk '{print $1}'
}

file_hash() {
  shasum -a 256 "$1" | awk '{print $1}'
}

assert_resources_arsc_stored() {
  local package="$1"
  local stock="$2"
  local out="$3"
  python3 - "$package" "$stock" "$out" <<'PY'
from __future__ import annotations

import sys
import zipfile

package, stock, out = sys.argv[1:4]
for label, path in [("stock", stock), ("output", out)]:
    with zipfile.ZipFile(path) as zf:
        try:
            info = zf.getinfo("resources.arsc")
        except KeyError:
            raise SystemExit(f"error: {package} {label} missing resources.arsc")
        if info.compress_type != zipfile.ZIP_STORED:
            raise SystemExit(
                f"error: {package} {label} resources.arsc is not STORED: "
                f"compress_type={info.compress_type}"
            )
PY
}

assert_eq() {
  local actual="$1"
  local expected="$2"
  local label="$3"
  [ "$actual" = "$expected" ] || {
    echo "error: ${label}: expected ${expected}, got ${actual}" >&2
    exit 1
  }
}

verify_apk() {
  local package="$1"
  local stock_rel="$2"
  local out_rel="$3"
  local expected_sha="$4"
  local variant="$5"
  local gate="$6"
  local reason="$7"
  local stock="${ROOT_DIR}/${stock_rel}"
  local out="${ROOT_DIR}/${out_rel}"
  local signature="${out%.apk}.signature.txt"
  local out_sha stock_entry out_entry policy_out

  need_file "$stock"
  need_file "$out"
  need_file "$signature"

  out_sha="$(file_hash "$out")"
  assert_eq "$out_sha" "$expected_sha" "${package} APK sha256"

  unzip -t "$out" >/dev/null

  for entry in classes.dex AndroidManifest.xml; do
    if entry_exists "$stock" "$entry"; then
      entry_exists "$out" "$entry" || {
        echo "error: ${package} output is missing ${entry}" >&2
        exit 1
      }
      stock_entry="$(entry_hash "$stock" "$entry")"
      out_entry="$(entry_hash "$out" "$entry")"
      assert_eq "$out_entry" "$stock_entry" "${package} ${entry}"
    fi
  done

  if entry_exists "$stock" resources.arsc && entry_exists "$out" resources.arsc; then
    stock_entry="$(entry_hash "$stock" resources.arsc)"
    out_entry="$(entry_hash "$out" resources.arsc)"
    [ "$stock_entry" != "$out_entry" ] || {
      echo "error: ${package} resources.arsc did not change" >&2
      exit 1
    }
    assert_resources_arsc_stored "$package" "$stock" "$out"
  else
    echo "error: ${package} resources.arsc missing in stock or output" >&2
    exit 1
  fi

  policy_out="$("${POLICY}" --keep-languages en,zh "$out")"
  grep -q "bad_locale_chunk_count=0" <<<"$policy_out" || {
    echo "error: ${package} locale policy failed" >&2
    echo "$policy_out" >&2
    exit 1
  }

  grep -q "SHA-256 digest error for resources.arsc" "$signature" || {
    echo "error: ${package} signature boundary did not show resources.arsc digest error" >&2
    exit 1
  }

  {
    echo "package=${package}"
    echo "variant=${variant}"
    echo "gate=${gate}"
    echo "reason=${reason}"
    echo "stock=${stock_rel}"
    echo "output=${out_rel}"
    echo "sha256=${out_sha}"
    echo "zip_integrity=ok"
    echo "classes_and_manifest=unchanged"
    echo "resources_arsc=changed"
    echo "resources_arsc_zip_method=stored"
    echo "signature_boundary=resources.arsc_digest_error"
    echo "$policy_out"
    echo
  } >> "$REPORT"
}

if [ "${1:-}" = "--help" ]; then
  usage
  exit 0
fi

need_file "$COVERAGE_TSV"
need_file "$POLICY"
mkdir -p "$INSPECT_DIR"
timestamp="$(date +%Y%m%d-%H%M%S)"
REPORT="${INSPECT_DIR}/verify-apk-only-locale-prune-candidates-${timestamp}.txt"

{
  echo "# APK-only locale-prune candidate verification"
  echo "timestamp=${timestamp}"
  echo "coverage_tsv=${COVERAGE_TSV#${ROOT_DIR}/}"
  echo "boundary=read-only; no ROM image, no flash, no reboot, no /data mutation"
  echo
} > "$REPORT"

count=0
while IFS=$'\t' read -r package stock_rel out_rel expected_sha variant gate reason; do
  [ -n "$package" ] || continue
  verify_apk "$package" "$stock_rel" "$out_rel" "$expected_sha" "$variant" "$gate" "$reason"
  count=$((count + 1))
done < <(
  awk -F '\t' '
    NR == 1 {
      for (i = 1; i <= NF; i++) {
        key = $i
        sub(/\r$/, "", key)
        h[key] = i
      }
      next
    }
    $(h["apk_only_apk"]) != "" {
      gate = ""
      reason = ""
      if ("exposure_gate" in h) {
        gate = $(h["exposure_gate"])
      } else if ("tier1_gate" in h) {
        gate = $(h["tier1_gate"])
      }
      if ("exposure_reason" in h) {
        reason = $(h["exposure_reason"])
      } else if ("tier1_gate_reason" in h) {
        reason = $(h["tier1_gate_reason"])
      }
      sub(/\r$/, "", reason)
      print \
        $(h["package"]) "\t" \
        "reverse/smartisan-8.5.3-rom-static/raw/" $(h["partition"]) "/" $(h["rel_path"]) "\t" \
        $(h["apk_only_apk"]) "\t" \
        $(h["apk_only_sha256"]) "\t" \
        $(h["apk_only_variant"]) "\t" \
        gate "\t" \
        reason
    }
  ' "$COVERAGE_TSV"
)

[ "$count" -gt 0 ] || {
  echo "error: no apk_only candidates found in ${COVERAGE_TSV}" >&2
  exit 1
}

{
  echo "apk_only_candidate_count=${count}"
  echo "result=PASS_OFFLINE_APK_ONLY_BATCH"
} >> "$REPORT"

echo "PASS: apk-only locale-prune candidate verification"
echo "count=${count}"
echo "report=${REPORT}"
