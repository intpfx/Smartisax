...
SOURCE_SYSTEM_B_WORK="${ROOT_DIR}/hard-rom/work/v0.portal5w-quiet-presentation/source/system-otatrust-v0.portal5s-event-time-input-priority.img"
...
export PACKAGE_DIR_MTIME_HEX="${PACKAGE_DIR_MTIME_HEX:-0x6a446000}"
export PACKAGE_DIR_MTIME_NOTE="${PACKAGE_DIR_MTIME_NOTE:-2026-07-01 06:00:00 +0800; invalidates Smartisax package scan/cache after Portal quiet presentation repair}"
export PURPOSE="${PURPOSE:-Reduce Chrome presentation/RVFC gaps by running the Portal and strict smoke video path in a quiet presentation surface, suppressing DOM log churn during WebRTC playback, and reporting RAF main-thread drift beside RVFC cadence.}"
export RESULT_NAME="${RESULT_NAME:-PASS_BUILD_V0PORTAL5W_QUIET_PRESENTATION}"
