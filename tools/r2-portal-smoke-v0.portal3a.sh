...
usage() {
  cat <<'USAGE'
Usage:
  tools/r2-portal-smoke-v0.portal3a.sh --url http://<r2-ip>:37601 --code <pairing-code>
...
print("status_portal_version=0.5.0")
print("status_webrtc=capability-probe")
...
if data.get("portalVersion") != "0.5.0":
    raise SystemExit("capabilities portalVersion mismatch")
if data.get("variant") != "v0.portal3a-webrtc-capability-probe":
...
PY
  ...
  echo "result=PORTAL_SMOKE_V0PORTAL3A_COMPLETED"
} 2>&1 | tee "$REPORT"
