...
SOURCE_SPARSE="${SOURCE_SPARSE:-${ROOT_DIR}/hard-rom/build/super-otatrust-v0.17-all-apk-only-locale-prune-exact-current.sparse.img}"
SOURCE_SHA256="942da9469ccf9a24ff390912f26d76673415d2a500482d060a89c11847faf819"
WORK_DIR="${WORK_DIR:-${ROOT_DIR}/hard-rom/work/v0.22-all-apk-only-locale-prune}"
OUT_DIR="${ROOT_DIR}/hard-rom/build"
APK_OUT_DIR="${OUT_DIR}/apk"
SYSTEM_IMG="${OUT_DIR}/system-otatrust-v0.22-all-apk-only-locale-prune.img"
OUT_SPARSE="${OUT_DIR}/super-otatrust-v0.22-all-apk-only-locale-prune-exact-current.sparse.img"
MANIFEST="${OUT_DIR}/super-otatrust-v0.22-all-apk-only-locale-prune-exact-current.SHA256SUMS.txt"
...
Build an offline v0.22 exact-current candidate from the verified v0.17-all
sparse super. It keeps the seven v0.17 APK-only promotions and adds the three
newer system_b APK-only candidates:

  /system/app/CompanionDeviceManager/CompanionDeviceManager.apk
  /system/app/SmartisanShareBrowser/SmartisanShareBrowser.apk
  /system/app/TrackerSmartisan/TrackerSmartisan.apk

The script uses the shared_blocks-safe held-stock-inode replacement pattern for
the three new system_b files and then rewrites system_b into a sparse super. It
never flashes, reboots, erases misc, or changes /data. Flashing still requires
...
replace_file_in_image() {
  ...
  dir="$(dirname "$dst")"
  base="$(basename "$dst")"
  temp_path="${dir}/.${base}.smartisax-v022-tmp"
  held_path="${dir}/.${base}.smartisax-v022-stock-held"
...
COMPANION_APK="${APK_OUT_DIR}/com.android.companiondevicemanager-locale-prune-en-zh.apk"
SHARE_BROWSER_APK="${APK_OUT_DIR}/com.smartisanos.share.browser-locale-prune-en-zh.apk"
TRACKER_APK="${APK_OUT_DIR}/com.smartisanos.tracker-locale-prune-en-zh.apk"

for apk in "$COMPANION_APK" "$SHARE_BROWSER_APK" "$TRACKER_APK"; do
  need_file "$apk"
done
...
echo "Extracting system_b from v0.17-all sparse super..."
...
echo "Replacing new APK-only candidates in system_b..."
: > "${WORK_DIR}/replacements.tsv"
replace_file_in_image "$SYSTEM_IMG" "$COMPANION_APK" \
  "/system/app/CompanionDeviceManager/CompanionDeviceManager.apk" \
  "system-companiondevicemanager" >> "${WORK_DIR}/replacements.tsv"
replace_file_in_image "$SYSTEM_IMG" "$SHARE_BROWSER_APK" \
  "/system/app/SmartisanShareBrowser/SmartisanShareBrowser.apk" \
  "system-smartisan-share-browser" >> "${WORK_DIR}/replacements.tsv"
replace_file_in_image "$SYSTEM_IMG" "$TRACKER_APK" \
  "/system/app/TrackerSmartisan/TrackerSmartisan.apk" \
  "system-tracker" >> "${WORK_DIR}/replacements.tsv"

echo "Checking modified ext4 image..."
fsck_image "$SYSTEM_IMG"

echo "Verifying replaced APKs after fsck..."
verify_file_in_image_after_fsck "$SYSTEM_IMG" "$COMPANION_APK" \
  "/system/app/CompanionDeviceManager/CompanionDeviceManager.apk" "system-companiondevicemanager"
verify_file_in_image_after_fsck "$SYSTEM_IMG" "$SHARE_BROWSER_APK" \
  "/system/app/SmartisanShareBrowser/SmartisanShareBrowser.apk" "system-smartisan-share-browser"
verify_file_in_image_after_fsck "$SYSTEM_IMG" "$TRACKER_APK" \
  "/system/app/TrackerSmartisan/TrackerSmartisan.apk" "system-tracker"
...
"$SPARSE_TOOL" \
  ...
  --variant "otatrust-v0.22-all-apk-only-locale-prune-exact-current"

system_b_hash="$(sha256_one "$SYSTEM_IMG")"
super_hash="$(sha256_one "$OUT_SPARSE")"

{
  echo "variant=otatrust-v0.22-all-apk-only-locale-prune-exact-current"
  echo "purpose=Promote all ten APK-only English/Chinese resources.arsc hard-prune candidates into a combined flashable sparse super"
