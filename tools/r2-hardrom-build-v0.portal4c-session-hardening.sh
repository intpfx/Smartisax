...
SOURCE_VARIANT="${SOURCE_VARIANT:-v0.portal4b-mp4-control-polish}"
SOURCE_SPARSE="${SOURCE_SPARSE:-${ROOT_DIR}/hard-rom/build/super-otatrust-v0.portal4b-mp4-control-polish.sparse.img}"
SOURCE_SPARSE_SHA256="${SOURCE_SPARSE_SHA256:-2a1b702184d351dc5b74b139f1b2961fb429702d7f857865a07680b3277d9fa6}"
SOURCE_SYSTEM_B="${SOURCE_SYSTEM_B:-${ROOT_DIR}/hard-rom/build/system-otatrust-v0.portal4b-mp4-control-polish.img}"
SOURCE_SYSTEM_B_SHA256="${SOURCE_SYSTEM_B_SHA256:-2c06a8295b4fb629464ed28190b4546774e444ed5982b1b9e054be9feb2a0826}"
...
SMARTISAX_LIB_DIR="${NEW_SMARTISAX_DIR}/lib"
SMARTISAX_ARM64_LIB_DIR="${SMARTISAX_LIB_DIR}/arm64"
SMARTISAX_ARM_LIB_DIR="${SMARTISAX_LIB_DIR}/arm"
WEBRTC_ARM64_SO="${WEBRTC_ARM64_SO:-}"
WEBRTC_ARM_SO="${WEBRTC_ARM_SO:-}"
WEBRTC_ARM64_SO_PATH="${SMARTISAX_ARM64_LIB_DIR}/libjingle_peerconnection_so.so"
WEBRTC_ARM_SO_PATH="${SMARTISAX_ARM_LIB_DIR}/libjingle_peerconnection_so.so"
...
BASE_SERVICES_JAR_SHA256="${BASE_SERVICES_JAR_SHA256:-${EXPECTED_SERVICES_JAR_SHA256}}"
SERVICES_JAR_CANDIDATE="${SERVICES_JAR_CANDIDATE:-}"
SERVICES_JAR_CANDIDATE_SHA256="${SERVICES_JAR_CANDIDATE_SHA256:-}"
...
PACKAGE_DIR_MTIME_HEX="${PACKAGE_DIR_MTIME_HEX:-0x6a3a9fd0}"
PACKAGE_DIR_MTIME_NOTE="${PACKAGE_DIR_MTIME_NOTE:-2026-06-24 00:00:00 +0800; invalidates Smartisax package scan/cache after Portal session/access-control hardening APK update}"

PURPOSE="${PURPOSE:-Harden the Smartisax Portal direct-LAN MP4 fallback with pairing-code rotation, bad-pair lockout, session status, security headers, and a browser-side local-session clear action on top of live-proven v0.portal4b.}"
RESULT_NAME="${RESULT_NAME:-PASS_BUILD_V0PORTAL4C_SESSION_HARDENING}"
...
Builds a system_b-only Smartisax Portal candidate on top of v0.portal4b. It
updates only the Smartisax priv-app APK and privapp whitelist, keeps the narrow
READ_FRAME_BUFFER services.jar policy unchanged, keeps token-gated
/api/webrtc/offer, /api/rtp/h264, /api/video/mp4, PNG screen, and privileged
input intact, and hardens pairing/session handling for the direct-LAN Portal.
It rebuilds system_b AVB/FEC roots=2 and patches that system_b image into the
v0.portal4b sparse super. It does not touch a live device.
USAGE
}
...
ensure_image_dir() {
  ...
  local dir="$2"
  local tag="$3"
  local cmd_file="${WORK_DIR}/mkdir-${tag}.debugfs"
  if debugfs_path_exists "$image" "$dir"; then
    return 0
  fi
  {
    echo "mkdir ${dir}"
    set_inode_common "$dir" "040755"
  } > "$cmd_file"
  "$DEBUGFS" -w -f "$cmd_file" "$image" >/dev/null
  debugfs_path_exists "$image" "$dir" || die "failed to create directory: ${dir}"
}

replace_smartisax_privapp_and_permissions() {
  local image="$1" cmd_file="${WORK_DIR}/replace-smartisax-privapp.debugfs"
  ...
  ! debugfs_path_exists "$image" "$OLD_SMARTISAX_DIR" || die "old Smartisax system-app path unexpectedly exists: ${OLD_SMARTISAX_DIR}"
  debugfs_path_exists "$image" "$NEW_SMARTISAX_DIR" || die "missing Smartisax priv-app directory: ${NEW_SMARTISAX_DIR}"
  debugfs_path_exists "$image" "$NEW_SMARTISAX_APK_PATH" || die "missing old Smartisax priv-app APK: ${NEW_SMARTISAX_APK_PATH}"
  debugfs_path_exists "$image" "$PRIVAPP_XML_PATH" || die "missing Smartisax privapp permission XML: ${PRIVAPP_XML_PATH}"

  debugfs_dump "$image" "$NEW_SMARTISAX_APK_PATH" "${WORK_DIR}/smartisax-old-priv-app.apk"
  debugfs_dump "$image" "$PRIVAPP_XML_PATH" "${WORK_DIR}/privapp-permissions-com.smartisax.browser-before.xml"

  {
    echo "rm ${NEW_SMARTISAX_APK_PATH}"
    echo "rm ${PRIVAPP_XML_PATH}"
...
install_smartisax_webrtc_system_libs() {
  local image="$1" cmd_file="${WORK_DIR}/install-smartisax-webrtc-libs.debugfs"
  [ -n "$WEBRTC_ARM64_SO" ] || return 0
  [ -n "$WEBRTC_ARM_SO" ] || die "WEBRTC_ARM_SO is required when WEBRTC_ARM64_SO is set"
  need_file "$WEBRTC_ARM64_SO"
  need_file "$WEBRTC_ARM_SO"

  ensure_image_dir "$image" "$SMARTISAX_LIB_DIR" "smartisax-lib"
  ensure_image_dir "$image" "$SMARTISAX_ARM64_LIB_DIR" "smartisax-lib-arm64"
  ensure_image_dir "$image" "$SMARTISAX_ARM_LIB_DIR" "smartisax-lib-arm"

  {
    if debugfs_path_exists "$image" "$WEBRTC_ARM64_SO_PATH"; then
      echo "rm ${WEBRTC_ARM64_SO_PATH}"
    fi
    if debugfs_path_exists "$image" "$WEBRTC_ARM_SO_PATH"; then
      echo "rm ${WEBRTC_ARM_SO_PATH}"
    fi
    echo "write ${WEBRTC_ARM64_SO} ${WEBRTC_ARM64_SO_PATH}"
    set_inode_common "$WEBRTC_ARM64_SO_PATH" "0100644"
    echo "write ${WEBRTC_ARM_SO} ${WEBRTC_ARM_SO_PATH}"
    set_inode_common "$WEBRTC_ARM_SO_PATH" "0100644"
  } > "$cmd_file"
  "$DEBUGFS" -w -f "$cmd_file" "$image" >/dev/null
  verify_image_file_hash "$image" "$WEBRTC_ARM64_SO_PATH" "$(sha256_one "$WEBRTC_ARM64_SO")" "smartisax-libjingle-arm64.so"
  verify_image_file_hash "$image" "$WEBRTC_ARM_SO_PATH" "$(sha256_one "$WEBRTC_ARM_SO")" "smartisax-libjingle-arm.so"
}
...
replace_services_jar() {
  local image="$1" cmd_file="${WORK_DIR}/replace-services-jar.debugfs" dumped="${WORK_DIR}/services-jar-public-after.jar"
  ...
  assert_unique_blocks_for_delete "$image" "$SERVICES_JAR_PATH" "services-jar-wadb1"
...
WEBRTC_ARM64_SO_SHA256=""
WEBRTC_ARM_SO_SHA256=""
if [ -n "$WEBRTC_ARM64_SO" ]; then
  need_file "$WEBRTC_ARM64_SO"
  need_file "$WEBRTC_ARM_SO"
...
rm -f "${WORK_DIR}"/*.debugfs "${WORK_DIR}"/*.apk "${WORK_DIR}"/*.xml "${WORK_DIR}"/*.jar "${WORK_DIR}"/*.txt

{
  ...
  echo "expected_services_jar_sha256=${EXPECTED_SERVICES_JAR_SHA256}"
  if [ -n "$SERVICES_JAR_CANDIDATE" ]; then
  ...
  verify_image_path_absent "$SYSTEM_B_IMG" "$OLD_SMARTISAX_DIR" "old-smartisax-system-app-before"
  verify_image_file_hash "$SYSTEM_B_IMG" "$SERVICES_JAR_PATH" "$EXPECTED_SERVICES_JAR_SHA256" "services-jar-before.jar"
  ...
  replace_smartisax_privapp_and_permissions "$SYSTEM_B_IMG"
  install_smartisax_webrtc_system_libs "$SYSTEM_B_IMG"
  if [ -n "$SERVICES_JAR_CANDIDATE" ]; then
    need_file "$SERVICES_JAR_CANDIDATE"
    [ -n "$SERVICES_JAR_CANDIDATE_SHA256" ] || die "SERVICES_JAR_CANDIDATE_SHA256 is required when SERVICES_JAR_CANDIDATE is set"
    [ "$(sha256_one "$SERVICES_JAR_CANDIDATE")" = "$SERVICES_JAR_CANDIDATE_SHA256" ] || die "services.jar candidate hash mismatch"
    verify_services_jar_delta "${WORK_DIR}/services-jar-before.jar" "$SERVICES_JAR_CANDIDATE" "${WORK_DIR}/services-jar-delta-report.txt"
    replace_services_jar "$SYSTEM_B_IMG"
  fi
  old_smartisax_sha256="$(sha256_one "${WORK_DIR}/smartisax-old-priv-app.apk")"
  old_privapp_xml_sha256="$(sha256_one "${WORK_DIR}/privapp-permissions-com.smartisax.browser-before.xml")"
  {
  ...
  final_services_jar_sha256="$EXPECTED_SERVICES_JAR_SHA256"
  services_jar_changed_entries="none"
  if [ -n "$SERVICES_JAR_CANDIDATE" ]; then
    final_services_jar_sha256="$SERVICES_JAR_CANDIDATE_SHA256"
    services_jar_changed_entries="$(awk -F= '$1=="changed_entries"{print $2; exit}' "${WORK_DIR}/services-jar-delta-report.txt")"
    [ -n "$services_jar_changed_entries" ] || services_jar_changed_entries="classes.dex"
  fi
  verify_image_file_hash "$SYSTEM_B_IMG" "$SERVICES_JAR_PATH" "$final_services_jar_sha256" "services-jar-final.jar"
  ...
  if [ -n "$WEBRTC_ARM64_SO" ]; then
    verify_image_file_hash "$SYSTEM_B_IMG" "$WEBRTC_ARM64_SO_PATH" "$WEBRTC_ARM64_SO_SHA256" "smartisax-libjingle-arm64-after-fsck.so"
    verify_image_file_hash "$SYSTEM_B_IMG" "$WEBRTC_ARM_SO_PATH" "$WEBRTC_ARM_SO_SHA256" "smartisax-libjingle-arm-after-fsck.so"
  fi
  verify_image_file_hash "$SYSTEM_B_IMG" "$SERVICES_JAR_PATH" "$final_services_jar_sha256" "services-jar-final-after-fsck.jar"
  ...
  echo "old_smartisax_priv_app_sha256=${old_smartisax_sha256}"
  echo "old_privapp_xml_sha256=${old_privapp_xml_sha256}"
  ...
  system_hash="$(sha256_one "$SYSTEM_B_IMG")"
  {
    ...
    echo "smartisax_apk=${SMARTISAX_APK}"
    echo "smartisax_apk_sha256=${SMARTISAX_APK_SHA256}"
    echo "smartisax_old_priv_app_sha256=${old_smartisax_sha256}"
    echo "privapp_xml_path=${PRIVAPP_XML_PATH}"
    echo "privapp_xml=${PRIVAPP_XML}"
    echo "privapp_xml_sha256=${PRIVAPP_XML_SHA256}"
    echo "privapp_xml_old_sha256=${old_privapp_xml_sha256}"
    if [ -n "$WEBRTC_ARM64_SO" ]; then
      echo "webrtc_arm64_so=${WEBRTC_ARM64_SO}"
      echo "webrtc_arm64_so_path=${WEBRTC_ARM64_SO_PATH}"
      echo "webrtc_arm64_so_sha256=${WEBRTC_ARM64_SO_SHA256}"
      echo "webrtc_arm_so=${WEBRTC_ARM_SO}"
      echo "webrtc_arm_so_path=${WEBRTC_ARM_SO_PATH}"
      echo "webrtc_arm_so_sha256=${WEBRTC_ARM_SO_SHA256}"
    fi
    echo "services_jar_path=${SERVICES_JAR_PATH}"
    echo "services_jar_base_sha256=${EXPECTED_SERVICES_JAR_SHA256}"
    echo "services_jar_final_sha256=${final_services_jar_sha256}"
    echo "services_jar_changed_entries=${services_jar_changed_entries}"
    if [ -n "$SERVICES_JAR_CANDIDATE" ]; then
      echo "services_jar_candidate=${SERVICES_JAR_CANDIDATE}"
      echo "services_jar_candidate_sha256=${SERVICES_JAR_CANDIDATE_SHA256}"
      echo "services_jar_delta_report=${WORK_DIR}/services-jar-delta-report.txt"
      cat "${WORK_DIR}/services-jar-delta-report.txt"
    fi
