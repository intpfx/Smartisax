...
export EXPECTED_VERSION_CODE="53"
export EXPECTED_VERSION_NAME="0.7.2"
export EXPECTED_NATIVE_SYSTEM_WEBRTC_LIBS="1"
export EXPECTED_PORTAL_VARIANT_MARKER="v0.agent0.2-one-step"
...
export OFFLINE_RESULT_NAME="PASS_OFFLINE_IMAGE_V0AGENT02_ONE_STEP"
export READ_ONLY_RESULT_NAME="PASS_READ_ONLY_V0AGENT02_ONE_STEP"
...
{
  echo
  echo "## agent0.2 extra offline checks"
  ...
  grep -R -q 'v0.agent0.2-one-step' "$decode_dir" || { echo "error: agent0.2 variant marker missing" >&2; exit 1; }
  ...
  echo "agent02_extra_offline_checks=ok"
} 2>&1 | tee -a "$report"
