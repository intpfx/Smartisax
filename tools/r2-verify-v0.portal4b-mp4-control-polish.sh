...
usage() {
  cat <<'USAGE'
Usage:
  tools/r2-verify-v0.portal4b-mp4-control-polish.sh --offline-image
  tools/r2-verify-v0.portal4b-mp4-control-polish.sh --read-only
...
verify_apk_semantics() {
  ...
  grep -q "package: name='com.smartisax.browser' versionCode='14' versionName='0.5.4'" "${WORK_DIR}/smartisax-aapt-badging.txt" \
  ...
  grep -R -q 'signaling-rtp-probe-mp4-live-polish' "${decode_dir}/smali" || die "portal WebRTC/RTP probe marker missing"
  grep -R -q 'mp4-live-loop-fallback' "${decode_dir}/smali" || die "portal MP4 live fallback marker missing"
  grep -R -q 'v0.portal4b-mp4-control-polish' "${decode_dir}/smali" || die "portal4b variant marker missing"
...
run_offline_image() {
  ...
  local report="${INSPECT_DIR}/verify-${VARIANT}-offline-image-$(date '+%Y%m%d-%H%M%S').txt"
  {
    ...
    echo "result=PASS_OFFLINE_IMAGE_V0PORTAL4B_MP4_CONTROL_POLISH"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
...
printf "init.svc.bootanim=%s\n" "$(getprop init.svc.bootanim)"'
    ...
    grep -q 'versionCode=14 ' <<<"$package_dump" || die "Smartisax PackageManager did not parse versionCode=14"
    ...
    echo "result=PASS_READ_ONLY_V0PORTAL4B_MP4_CONTROL_POLISH"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
