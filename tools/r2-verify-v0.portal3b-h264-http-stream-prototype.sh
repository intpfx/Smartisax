...
usage() {
  cat <<'USAGE'
Usage:
  tools/r2-verify-v0.portal3b-h264-http-stream-prototype.sh --offline-image
  tools/r2-verify-v0.portal3b-h264-http-stream-prototype.sh --read-only
...
verify_apk_semantics() {
  ...
  grep -q "package: name='com.smartisax.browser' versionCode='11' versionName='0.5.1'" "${WORK_DIR}/smartisax-aapt-badging.txt" \
  ...
  grep -R -q 'h264-http-prototype' "${decode_dir}/smali" || die "portal H.264 stream marker missing"
  grep -R -q 'v0.portal3b-h264-http-stream-prototype' "${decode_dir}/smali" || die "portal3b variant marker missing"
  ...
  grep -R -q 'RTCRtpSender' "${decode_dir}/smali" || die "browser RTCRtpSender capability probe missing"
  grep -R -q 'VideoEncoder' "${decode_dir}/smali" || die "browser WebCodecs VideoEncoder probe missing"
...
run_offline_image() {
  ...
  local report="${INSPECT_DIR}/verify-${VARIANT}-offline-image-$(date '+%Y%m%d-%H%M%S').txt"
  {
    ...
    echo "result=PASS_OFFLINE_IMAGE_V0PORTAL3B_H264_HTTP_STREAM_PROTOTYPE"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
...
printf "init.svc.bootanim=%s\n" "$(getprop init.svc.bootanim)"'
    ...
    grep -q 'versionCode=11 ' <<<"$package_dump" || die "Smartisax PackageManager did not parse versionCode=11"
    ...
    echo "result=PASS_READ_ONLY_V0PORTAL3B_H264_HTTP_STREAM_PROTOTYPE"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
