...
APK_BUILDER="${APK_BUILDER:-${ROOT_DIR}/tools/r2-build-settingssmartisan-locale-filter-apk.sh}"
...
WORK_DIR="${ROOT_DIR}/hard-rom/work/v0.7-locale-filter"
OUT_DIR="${ROOT_DIR}/hard-rom/build"
APK_OUT_DIR="${OUT_DIR}/apk"
RAW_SUPER="${WORK_DIR}/super-otatrust-v0.7-locale-filter-exact-current.img"
SYSTEM_IMG="${OUT_DIR}/system-otatrust-v0.7-locale-filter.img"
OUT_SPARSE="${OUT_DIR}/super-otatrust-v0.7-locale-filter-exact-current.sparse.img"
MANIFEST="${OUT_DIR}/super-otatrust-v0.7-locale-filter-exact-current.SHA256SUMS.txt"
LOCALE_APK="${APK_OUT_DIR}/SettingsSmartisan-locale-filter-ja-ko.apk"
LOCALE_SIG_REPORT="${APK_OUT_DIR}/SettingsSmartisan-locale-filter-ja-ko.signature.txt"
DUMPED_LOCALE_APK="${WORK_DIR}/SettingsSmartisan-locale-filter-dumped-from-system.img.apk"
...
Build an offline v0.7 exact-current super candidate from the stable v0.4
baseline. The ROM change is replacing SettingsSmartisan.apk with a behavior
patch that hides ja_JP and ko_KR in Smartisan's visible language picker.
...
has passed live validation and the user explicitly confirms flashing v0.7.
USAGE
}
...
replace_file_in_image() {
  ...
  dir="$(dirname "$dst")"
  base="$(basename "$dst")"
  temp_path="${dir}/.${base}.smartisax-v07-tmp"
  held_path="${dir}/.${base}.smartisax-v07-stock-held"
...
echo "Building SettingsSmartisan locale-filter APK..."
"$APK_BUILDER" >/dev/null
need_file "$LOCALE_APK"
need_file "$LOCALE_SIG_REPORT"
grep -q '^keytool_status=1$' "$LOCALE_SIG_REPORT" \
  || die "unexpected locale APK keytool boundary; review ${LOCALE_SIG_REPORT}"
grep -q 'SHA-256 digest error for classes.dex' "$LOCALE_SIG_REPORT" \
  || die "locale APK signature report does not show the expected classes.dex digest boundary"
...
replace_file_in_image "$SYSTEM_IMG" "$LOCALE_APK" \
  "$SETTINGS_APK_DST" "$SELABEL" \
  "settingssmartisan-locale-filter" >> "${WORK_DIR}/replacements.tsv"

echo "Checking modified system image..."
fsck_image "$SYSTEM_IMG"

echo "Verifying replaced APK after fsck..."
verify_file_in_image_after_fsck "$SYSTEM_IMG" "$LOCALE_APK" \
  "$SETTINGS_APK_DST" \
  "settingssmartisan-locale-filter"

locale_hash="$(sha256_one "$LOCALE_APK")"
dumped_hash="$(sha256_one "${WORK_DIR}/settingssmartisan-locale-filter-postfsck-dumped.apk")"
...
{
  echo "variant=otatrust-v0.7-locale-filter-exact-current"
  echo "purpose=SettingsSmartisan ja_JP/ko_KR visible language picker filter"
  ...
  echo "settings_locale_filter_apk=${LOCALE_APK}"
  echo "settings_locale_filter_signature_report=${LOCALE_SIG_REPORT}"
  echo "settings_locale_filter_inserted_path=${SETTINGS_APK_DST}"
  echo "settings_locale_filter_sha256=${locale_hash}"
  ...
  echo
  shasum -a 256 "$OUT_SPARSE" "$SYSTEM_IMG" "$BASE_SPARSE" "$LOCALE_APK" "$STOCK_SETTINGS_APK"
} > "$MANIFEST"

echo "Built: ${OUT_SPARSE}"
echo "System image: ${SYSTEM_IMG}"
echo "Locale APK: ${LOCALE_APK}"
echo "Signature report: ${LOCALE_SIG_REPORT}"
