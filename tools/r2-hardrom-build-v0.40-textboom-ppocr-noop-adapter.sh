...
TEXTBOOM_APK_BUILDER="${TEXTBOOM_APK_BUILDER:-${ROOT_DIR}/tools/r2-build-textboom-ppocr-noop-adapter-apk.sh}"
...
TEXTBOOM_ADAPTER_APK="${TEXTBOOM_ADAPTER_APK:-${ROOT_DIR}/hard-rom/build/apk/TextBoom-ppocr-noop-adapter.apk}"
TEXTBOOM_ADAPTER_APK_SHA256="e2f659aeb8db55003531f9b02360f1d49df4152a3d1cd59874e837ad797fc0d3"
...
PURPOSE="Switch TextBoom image OCR entrypoints to a LocalPpOcrApi no-op adapter on top of live-stable v0.39, preserving Sidebar font-OCR deletion, M150 WebView, Smartisax shell, TextBoom lib/arm, and system_b AVB/FEC."
...

{
  ...
  replace_file_in_image "$SYSTEM_B_IMG" "$TEXTBOOM_ADAPTER_APK" "$TEXTBOOM_PATH" \
    "textboom-ppocr-noop-adapter" >> "${WORK_DIR}/replacements.tsv"
  ...
  python3 "$AVBTOOL" info_image --image "$SYSTEM_B_IMG" > "${WORK_DIR}/system-b-v040-avb-info.txt"
  grep -q "FEC num roots:         2" "${WORK_DIR}/system-b-v040-avb-info.txt" || die "system_b lost FEC roots"
...

  {
    ...
    echo "textboom_adapter_behavior=noop_success_empty_list"
    echo "textboom_patched_entrypoints=BoomOcrActivity.initView,BoomAccessOcrActivity.initOcr"
    echo "textboom_changed_payloads=classes2.dex"
  ...
  echo "result=PASS_BUILD_V040_TEXTBOOM_PPOCR_NOOP_ADAPTER"
} 2>&1 | tee "$REPORT"
