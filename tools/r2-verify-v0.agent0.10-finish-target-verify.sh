...
export VARIANT="v0.agent0.10-finish-target-verify"
export EXPECTED_VERSION_CODE="61"
export EXPECTED_VERSION_NAME="0.7.10"
export EXPECTED_NATIVE_SYSTEM_WEBRTC_LIBS="1"
export EXPECTED_PORTAL_VARIANT_MARKER="v0.agent0.10-finish-target-verify"
...
export OFFLINE_RESULT_NAME="PASS_OFFLINE_IMAGE_V0AGENT010_FINISH_TARGET_VERIFY"
export READ_ONLY_RESULT_NAME="PASS_READ_ONLY_V0AGENT010_FINISH_TARGET_VERIFY"

SYSTEM_IMG="${ROOT_DIR}/hard-rom/build/system-otatrust-${VARIANT}.img"
EXPECTED_SYSTEM_SHA256="79508a660d2b412fba6eb94582f11aba4947ce4e2fa85e500d33742eb2ad2ca0"
...
{
  echo
  echo "## agent0.10 extra offline checks"
  ...
  grep -R -q 'v0.agent0.10-finish-target-verify' "$decode_dir" || { echo "error: agent0.10 variant marker missing" >&2; exit 1; }
  grep -R -q 'agent_worker_not_alive' "$decode_dir" || { echo "error: Agent worker reconciliation marker missing" >&2; exit 1; }
  grep -R -q 'accessibilityTargets' "$decode_dir" || { echo "error: accessibilityTargets status marker missing" >&2; exit 1; }
  grep -R -q 'oneStepAppNodeCount' "$decode_dir" || { echo "error: One Step target count marker missing" >&2; exit 1; }
  grep -R -q 'settingsNodeCount' "$decode_dir" || { echo "error: Settings target count marker missing" >&2; exit 1; }
  grep -R -q 'finishTargetVerification' "$decode_dir" || { echo "error: target-aware finish policy marker missing" >&2; exit 1; }
  grep -R -q 'finish_target_verified' "$decode_dir" || { echo "error: target-aware finish accepted marker missing" >&2; exit 1; }
  grep -R -q 'settings_target_visible' "$decode_dir" || { echo "error: Settings target finish marker missing" >&2; exit 1; }
  grep -R -q 'foregroundPackageMatched' "$decode_dir" || { echo "error: foreground target verification marker missing" >&2; exit 1; }
  grep -R -q 'accessibilityWindowMatched' "$decode_dir" || { echo "error: Accessibility window target marker missing" >&2; exit 1; }
  grep -R -q 'accessibilityPackageNodeMatched' "$decode_dir" || { echo "error: Accessibility package-node target marker missing" >&2; exit 1; }
  grep -R -q 'isSettings' "$decode_dir" || { echo "error: foreground Settings state marker missing" >&2; exit 1; }
  ...
  echo "agent010_extra_offline_checks=ok"
} 2>&1 | tee -a "$report"
