...
INSPECT_DIR="${ROOT_DIR}/hard-rom/inspect/v0.26c-sidebar-launcher-entry-hide-v2cert-cachebump"
EXPECTED_SUPER="${ROOT_DIR}/hard-rom/build/super-otatrust-v0.26c-sidebar-launcher-entry-hide-v2cert-cachebump-exact-current.sparse.img"
EXPECTED_SYSTEM_IMG="${ROOT_DIR}/hard-rom/build/system-otatrust-v0.26c-sidebar-launcher-entry-hide-v2cert-cachebump.img"
SOURCE_V026B="${ROOT_DIR}/hard-rom/build/super-otatrust-v0.26b-sara-launcher-entry-hide-v2cert-cachebump-exact-current.sparse.img"
V026B_REPORT_DIR="${ROOT_DIR}/hard-rom/inspect/v0.26b-sara-launcher-entry-hide-v2cert-cachebump"
VERIFY_LABEL="v0.26c Sidebar launcher-entry-hide v2cert cachebump"
REPORT_PREFIX="verify-v0.26c-sidebar-launcher-entry-hide-v2cert-cachebump"
...
SARA_STOCK_APK="${RAW}/system/system/priv-app/VoiceAssistant/VoiceAssistant.apk"
SIDEBAR_STOCK_APK="${RAW}/system/system/priv-app/Sidebar/Sidebar.apk"

VIDEO_APK="${ROOT_DIR}/hard-rom/build/apk/com.smartisanos.videoplayerproject-launcher-hidden-v2cert.apk"
SCREENREC_APK="${ROOT_DIR}/hard-rom/build/apk/com.smartisanos.screenrecorder-launcher-hidden-v2cert.apk"
QUICKSEARCH_APK="${ROOT_DIR}/hard-rom/build/apk/com.smartisanos.quicksearch-launcher-hidden-v2cert.apk"
SARA_APK="${ROOT_DIR}/hard-rom/build/apk/com.smartisanos.sara-launcher-hidden-v2cert.apk"
SIDEBAR_APK="${ROOT_DIR}/hard-rom/build/apk/com.smartisanos.sidebar-launcher-hidden-v2cert.apk"
APK_MANIFEST="${ROOT_DIR}/hard-rom/build/apk/launcher-entry-hide-apk-manifest-v0.26c.tsv"

V026A2_DIR_MTIME="0x6a33ddc0"
SARA_DIR_MTIME="0x6a33ebd0"
SIDEBAR_DIR_MTIME="0x6a33f9e0"

usage() {
  cat <<'USAGE'
Usage:
  tools/r2-verify-v0.26c-sidebar-launcher-entry-hide-v2cert-cachebump.sh --offline-image
  tools/r2-verify-v0.26c-sidebar-launcher-entry-hide-v2cert-cachebump.sh --read-only

--offline-image verifies the generated v0.26c sparse super:
  - v0.26b launcher-hidden APKs are retained
  - Sidebar changes only AndroidManifest.xml as expected
  - all edited APKs carry the stock APK signing-block certificate carrier
  - held-stock hidden paths and package directory mtime bumps exist
  - sparse system_b matches the generated system image
  - system_ext_b remains byte-identical to the v0.26b source
...
if expected != actual:
    Path("/tmp/r2-v026c-expected.xml").write_bytes(expected)
    Path("/tmp/r2-v026c-actual.xml").write_bytes(actual)
...
verify_apk_manifest_tsv() {
  ...
  grep -Fq $'一步\tcom.smartisanos.sidebar\tcom.smartisanos.sidebar.setting.SettingActivity\t1' "$APK_MANIFEST" \
    || die "APK manifest is missing Sidebar v0.26c row"
...
dump_and_verify_system_image() {
  ...
  dump_one "$image" "/system/priv-app/VoiceAssistant/VoiceAssistant.apk" "${dump_dir}/VoiceAssistant.apk"
  dump_one "$image" "/system/priv-app/Sidebar/Sidebar.apk" "${dump_dir}/Sidebar.apk"
  ...
  compare_file_hash "${dump_dir}/VoiceAssistant.apk" "$SARA_APK" "system/VoiceAssistant.apk"
  compare_file_hash "${dump_dir}/Sidebar.apk" "$SIDEBAR_APK" "system/Sidebar.apk"
  ...
  verify_apk_manifest_scope "$SARA_STOCK_APK" "${dump_dir}/VoiceAssistant.apk" \
    "com.smartisanos.sara" "com.smartisanos.sara.bubble.SettingActivity" "1" \
    "${dump_dir}/check-VoiceAssistant"
  verify_apk_manifest_scope "$SIDEBAR_STOCK_APK" "${dump_dir}/Sidebar.apk" \
    "com.smartisanos.sidebar" "com.smartisanos.sidebar.setting.SettingActivity" "1" \
    "${dump_dir}/check-Sidebar"
  echo
  ...
  echo
  verify_sig_boundary "${dump_dir}/VoiceAssistant.apk" "${dump_dir}/VoiceAssistant.signature.txt"
  echo
  verify_sig_boundary "${dump_dir}/Sidebar.apk" "${dump_dir}/Sidebar.signature.txt"
  echo

  echo "## held-stock paths"
  verify_held_path "$image" "/system/priv-app/VideoPlayer/.VideoPlayer.apk.smartisax-v026a2-stock-held"
  verify_held_path "$image" "/system/priv-app/ScreenRecorderSmartisan/.ScreenRecorderSmartisan.apk.smartisax-v026a2-stock-held"
  verify_held_path "$image" "/system/app/QuickSearchBoxSmartisan/.QuickSearchBoxSmartisan.apk.smartisax-v026a2-stock-held"
  verify_held_path "$image" "/system/priv-app/VoiceAssistant/.VoiceAssistant.apk.smartisax-v026b-stock-held"
  verify_held_path "$image" "/system/priv-app/Sidebar/.Sidebar.apk.smartisax-v026c-stock-held"
  echo

  echo "## package directory mtimes"
  verify_package_dir_mtime "$image" "/system/priv-app/VideoPlayer" "$V026A2_DIR_MTIME"
  verify_package_dir_mtime "$image" "/system/priv-app/ScreenRecorderSmartisan" "$V026A2_DIR_MTIME"
  verify_package_dir_mtime "$image" "/system/app/QuickSearchBoxSmartisan" "$V026A2_DIR_MTIME"
  verify_package_dir_mtime "$image" "/system/priv-app/VoiceAssistant" "$SARA_DIR_MTIME"
  verify_package_dir_mtime "$image" "/system/priv-app/Sidebar" "$SIDEBAR_DIR_MTIME"
}

verify_retained_system_ext() {
  local dump_dir="$1"
  local source_system_ext="${dump_dir}/system_ext_b-from-v0.26b.img"
  local output_system_ext="${dump_dir}/system_ext_b-from-output.img"
  "$SPARSE_TOOL" --source-sparse "$SOURCE_V026B" \
...
run_offline_image() {
  ...
  local v026b_report
  v026b_report="$(latest_report "$V026B_REPORT_DIR" "verify-v0.26b-sara-launcher-entry-hide-v2cert-cachebump-device-*.txt")"
  [ -n "$v026b_report" ] || die "missing v0.26b device PASS report"
  grep -Fq "PASS: v0.26b Sara launcher-entry-hide v2cert cachebump device read-only verification" "$v026b_report" \
    || die "latest v0.26b device report is not PASS: ${v026b_report}"
...

  {
    ...
    echo "source_v0.26b=${SOURCE_V026B}"
    echo "source_v0.26b_device_report=${v026b_report}"
    echo
    ...
    echo "## system_b inserted and retained APKs"
    ...
    echo "## hashes"
    shasum -a 256 "$EXPECTED_SUPER" "$EXPECTED_SYSTEM_IMG" "$SOURCE_V026B" \
      "$VIDEO_APK" "$SCREENREC_APK" "$QUICKSEARCH_APK" "$SARA_APK" "$SIDEBAR_APK" \
      "$VIDEO_STOCK_APK" "$SCREENREC_STOCK_APK" "$QUICKSEARCH_STOCK_APK" "$SARA_STOCK_APK" "$SIDEBAR_STOCK_APK"
    echo
    echo "result=PASS"
...
check_live_contains() {
  ...
  if grep -Fq "$needle" <<<"$haystack"; then
    ...
    note_live_failure "${label}: missing ${needle}"
  fi
}
...
printf "ro.build.fingerprint=%s\n" "$(getprop ro.build.fingerprint)"'
  ...
  verify_live_apk "system/VoiceAssistant.apk" "com.smartisanos.sara" "$SARA_APK"
  verify_live_apk "system/Sidebar.apk" "com.smartisanos.sidebar" "$SIDEBAR_APK"
  echo
  ...
  check_launcher_absent "VoiceAssistant" "com.smartisanos.sara" \
    "com.smartisanos.sara.bubble.SettingActivity" "$launcher_query"
  check_launcher_absent "Sidebar" "com.smartisanos.sidebar" \
    "com.smartisanos.sidebar.setting.SettingActivity" "$launcher_query"
  echo

  echo "## Sidebar preserved feature surface"
  local sidebar_pkg
  local sidebar_services
  local sidebar_windows
  local explicit_resolve
  sidebar_pkg="$(adb_shell 'dumpsys package com.smartisanos.sidebar | grep -E "Package \\[|sharedUser=|userId=|codePath=|apkSigningVersion=|SidebarService|SidebarCallProvider|SidebarSyncProvider|IdeaPillsSyncProvider|ForceTouchProvider|android.intent.action.BOOT_COMPLETED|com.smartisanos.sidebar.sticky.activity|smartisanos.intent.action.BOOM_FONT" | sed -n "1,180p" || true')"
  sidebar_services="$(adb_shell 'dumpsys activity services com.smartisanos.sidebar | sed -n "1,180p" || true')"
  sidebar_windows="$(adb_shell 'dumpsys window | grep -i sidebar | sed -n "1,140p" || true')"
  explicit_resolve="$(adb_shell 'cmd package resolve-activity --brief -n com.smartisanos.sidebar/.setting.SettingActivity 2>/dev/null || true')"
  printf '%s\n' "$sidebar_pkg"
  echo "-- explicit resolve --"
  printf '%s\n' "$explicit_resolve"
  echo "-- services --"
  printf '%s\n' "$sidebar_services"
  echo "-- windows --"
  ...
  check_live_contains "sidebar_shared_uid" "$sidebar_pkg" "sharedUser=SharedUserSetting"
  check_live_contains "sidebar_service_record" "$sidebar_services" "com.smartisanos.sidebar/.SidebarService"
  check_live_contains "sidebar_provider_call" "$sidebar_pkg" "SidebarCallProvider"
  check_live_contains "sidebar_provider_sync" "$sidebar_pkg" "SidebarSyncProvider"
  check_live_contains "sidebar_provider_idea_pills" "$sidebar_pkg" "IdeaPillsSyncProvider"
  check_live_contains "sidebar_provider_forcetouch" "$sidebar_pkg" "ForceTouchProvider"
  check_live_contains "sidebar_explicit_setting_activity" "$explicit_resolve" "com.smartisanos.sidebar/.setting.SettingActivity"
  check_live_contains "sidebar_service_bound" "$sidebar_services" "ServiceRecord"
  check_live_contains "sidebar_service_system_binding" "$sidebar_services" "com.smartisanos.sidebar/.SidebarService:system"
  check_live_contains "sidebar_window_content" "$sidebar_windows" "sidebar_content_area"
  check_live_contains "sidebar_window_top" "$sidebar_windows" "sidebar_top_area"
  check_live_contains "sidebar_window_side" "$sidebar_windows" "sidebar_side_area"
  echo

  echo "## relevant settings read-only"
  adb_shell 'settings get global side_bar_mode;
settings get global sidebar_switch_status;
settings get global side_bar_zoom_type;
settings get global squeeze_side_buttons_enabled;
settings get global squeeze_side_buttons' || true
  echo
