...
SOURCE_SPARSE="${SOURCE_SPARSE:-${ROOT_DIR}/hard-rom/build/super-otatrust-v0.26c-sidebar-launcher-entry-hide-v2cert-cachebump-exact-current.sparse.img}"
SOURCE_SHA256="${SOURCE_SHA256:-fa78ad42e8e8e367a61339d7bf28e4b94dba402bdfb02a944c317a1eda76c5e1}"

WORK_DIR="${WORK_DIR:-${ROOT_DIR}/hard-rom/work/v0.27-cloud-service-debloat}"
OUT_DIR="${ROOT_DIR}/hard-rom/build"
SYSTEM_IMG="${SYSTEM_IMG:-${OUT_DIR}/system-otatrust-v0.27-cloud-service-debloat.img}"
OUT_SPARSE="${OUT_SPARSE:-${OUT_DIR}/super-otatrust-v0.27-cloud-service-debloat-exact-current.sparse.img}"
MANIFEST="${MANIFEST:-${OUT_DIR}/super-otatrust-v0.27-cloud-service-debloat-exact-current.SHA256SUMS.txt}"
...
SPARSE_VARIANT="otatrust-v0.27-cloud-service-debloat-exact-current"

remove_entries=(
  "com.smartisanos.cloudsync|/system/priv-app/CloudServiceSmartisan|10112386|Smartisan account, sync adapters, find-phone, cloud backup login"
  "com.smartisanos.cloudsyncshare|/system/priv-app/CloudServiceShare|2558521|third-party login/share bridge for Smartisan cloud"
  "com.smartisanos.cloudagent|/system/priv-app/CloudSyncAgent|309723|find-phone/cloud system provider agent"
)
...
Build v0.27 on top of the live-verified v0.26c sparse image. The candidate
hard-removes the Smartisan cloud service ROM packages from system_b:

  /system/priv-app/CloudServiceSmartisan
  /system/priv-app/CloudServiceShare
  /system/priv-app/CloudSyncAgent

It also removes their entries from hiddenapi-package-whitelist.xml. The script
does not flash, reboot, erase misc, or change /data. The live device currently
has an updated-system /data/app copy of com.smartisanos.cloudsync, so final
package absence may require a separate user-confirmed /data cleanup after
flashing v0.27.
USAGE
}
...
replace_file_in_image() {
  ...
  dir="$(dirname "$dst")"
  base="$(basename "$dst")"
  temp_path="${dir}/.${base}.smartisax-v027-tmp"
  held_path="${dir}/.${base}.smartisax-v027-stock-held"
...
filter_hiddenapi_whitelist() {
  ...
  local filtered="${WORK_DIR}/hiddenapi-package-whitelist.v0.27.xml"
  local pattern
  ...
  if grep -Eq "$pattern" "$filtered"; then
    die "filtered hiddenapi whitelist still contains a cloud package"
  fi
...
echo "Extracting system_b from v0.26c sparse super..."
...
echo "Removing Smartisan cloud service package directories..."
...
echo "Removing cloud package entries from hiddenapi whitelist..."
...
for entry in "${remove_entries[@]}"; do
  IFS='|' read -r package_name remove_path _apk_bytes _note <<<"$entry"
  if debugfs_path_exists "$SYSTEM_IMG" "$remove_path"; then
...

{
  ...
  echo "purpose=Hard-remove Smartisan cloud service ROM packages on top of live-verified v0.26c"
  echo "flash_gate=not authorized; explicit user confirmation required"
  echo "data_cleanup_gate=updated-system /data/app com.smartisanos.cloudsync cleanup requires separate explicit user confirmation"
