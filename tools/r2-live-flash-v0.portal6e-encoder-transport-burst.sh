...
RESULT_NAME="PASS_FLASH_V0PORTAL6E_ENCODER_TRANSPORT_BURST"
IMAGE="${ROOT_DIR}/hard-rom/build/super-otatrust-v0.portal6e-encoder-transport-burst.sparse.img"
EXPECTED_IMAGE_SHA256_FILE="${ROOT_DIR}/hard-rom/build/super-otatrust-v0.portal6e-encoder-transport-burst.SHA256SUMS.txt"
CONFIRM_PHRASE="确认刷入 v0.portal6e-encoder-transport-burst B 槽"
INSPECT_DIR="${ROOT_DIR}/hard-rom/inspect/${VARIANT}"
VERIFY_SCRIPT="${ROOT_DIR}/tools/r2-verify-v0.portal6e-encoder-transport-burst.sh"
PREFLIGHT_SCRIPT="${ROOT_DIR}/tools/r2-live-flash-preflight.sh"

usage() {
  cat <<USAGE
Usage:
  tools/r2-live-flash-v0.portal6e-encoder-transport-burst.sh --confirm "${CONFIRM_PHRASE}"
...
read-only v0.portal6e verifier.
USAGE
}
