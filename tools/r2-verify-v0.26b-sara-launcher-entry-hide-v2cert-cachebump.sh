...
INSPECT_DIR="${ROOT_DIR}/hard-rom/inspect/v0.26b-sara-launcher-entry-hide-v2cert-cachebump"
EXPECTED_SUPER="${ROOT_DIR}/hard-rom/build/super-otatrust-v0.26b-sara-launcher-entry-hide-v2cert-cachebump-exact-current.sparse.img"
EXPECTED_SYSTEM_IMG="${ROOT_DIR}/hard-rom/build/system-otatrust-v0.26b-sara-launcher-entry-hide-v2cert-cachebump.img"
SOURCE_V026A2="${ROOT_DIR}/hard-rom/build/super-otatrust-v0.26a.2-launcher-entry-hide-v2cert-cachebump-exact-current.sparse.img"
V026A2_REPORT_DIR="${ROOT_DIR}/hard-rom/inspect/v0.26a.2-launcher-entry-hide-v2cert-cachebump"
VERIFY_LABEL="v0.26b Sara launcher-entry-hide v2cert cachebump"
REPORT_PREFIX="verify-v0.26b-sara-launcher-entry-hide-v2cert-cachebump"
...
APK_MANIFEST="${ROOT_DIR}/hard-rom/build/apk/launcher-entry-hide-apk-manifest-v0.26b.tsv"
...
usage() {
  cat <<'USAGE'
Usage:
  tools/r2-verify-v0.26b-sara-launcher-entry-hide-v2cert-cachebump.sh --offline-image
  tools/r2-verify-v0.26b-sara-launcher-entry-hide-v2cert-cachebump.sh --read-only

--offline-image verifies the generated v0.26b sparse super:
  - v0.26a.2 launcher-hidden APKs are retained
  - Sara/VoiceAssistant changes only AndroidManifest.xml as expected
  - all edited APKs carry the stock v2 signing-block certificate carrier
  - held-stock hidden paths and package directory mtime bumps exist
  - sparse system_b matches the generated system image
  - system_ext_b remains byte-identical to the v0.26a.2 source
...
if expected != actual:
    Path("/tmp/r2-v026b-expected.xml").write_bytes(expected)
    Path("/tmp/r2-v026b-actual.xml").write_bytes(actual)
...
verify_apk_manifest_tsv() {
  ...
  grep -Fq $'闪念胶囊\tcom.smartisanos.sara\tcom.smartisanos.sara.bubble.SettingActivity\t1' "$APK_MANIFEST" \
    || die "APK manifest is missing Sara v0.26b row"
...
verify_retained_system_ext() {
  local dump_dir="$1"
  local source_system_ext="${dump_dir}/system_ext_b-from-v0.26a.2.img"
  local output_system_ext="${dump_dir}/system_ext_b-from-output.img"
  "$SPARSE_TOOL" --source-sparse "$SOURCE_V026A2" \
...
run_offline_image() {
  ...
  local v026a2_report
  v026a2_report="$(latest_report "$V026A2_REPORT_DIR" "verify-v0.26a.2-launcher-entry-hide-v2cert-cachebump-device-*.txt")"
  [ -n "$v026a2_report" ] || die "missing v0.26a.2 device PASS report"
  grep -Fq "PASS: v0.26a.2 launcher-entry-hide v2cert cachebump device read-only verification" "$v026a2_report" \
    || die "latest v0.26a.2 device report is not PASS: ${v026a2_report}"
...

  {
    ...
    echo "source_v0.26a.2=${SOURCE_V026A2}"
    echo "source_v0.26a.2_device_report=${v026a2_report}"
    echo
    ...
    echo "## hashes"
    shasum -a 256 "$EXPECTED_SUPER" "$EXPECTED_SYSTEM_IMG" "$SOURCE_V026A2" \
      "$VIDEO_APK" "$SCREENREC_APK" "$QUICKSEARCH_APK" "$SARA_APK" \
      "$VIDEO_STOCK_APK" "$SCREENREC_STOCK_APK" "$QUICKSEARCH_STOCK_APK" "$SARA_STOCK_APK"
    echo
    echo "result=PASS"
...
printf "ro.build.fingerprint=%s\n" "$(getprop ro.build.fingerprint)"'
  ...
  adb_shell 'dumpsys package com.smartisanos.sara | grep -E "SettingActivity|ShortCutAppSettingActivity|DrawerSettingActivity|SaraProvider|ForceTouchProvider|SpeechLocationTimeProvider|OffLineRecognizeService|VoiceCommandActivity|launchShellBubbleSpeech|launchIatSpeech|android.app.shortcuts" | sed -n "1,120p"' || true
  echo
