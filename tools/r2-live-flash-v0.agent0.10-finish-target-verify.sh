...
RESULT_NAME="PASS_FLASH_V0AGENT010_FINISH_TARGET_VERIFY"
IMAGE="${ROOT_DIR}/hard-rom/build/super-otatrust-v0.agent0.10-finish-target-verify.sparse.img"
EXPECTED_IMAGE_SHA256_FILE="${ROOT_DIR}/hard-rom/build/super-otatrust-v0.agent0.10-finish-target-verify.SHA256SUMS.txt"
CONFIRM_PHRASE="确认刷入 v0.agent0.10-finish-target-verify B 槽"
INSPECT_DIR="${ROOT_DIR}/hard-rom/inspect/${VARIANT}"
VERIFY_SCRIPT="${ROOT_DIR}/tools/r2-verify-v0.agent0.10-finish-target-verify.sh"
PREFLIGHT_SCRIPT="${ROOT_DIR}/tools/r2-live-flash-preflight.sh"

usage() {
  cat <<USAGE
Usage:
  tools/r2-live-flash-v0.agent0.10-finish-target-verify.sh --confirm "${CONFIRM_PHRASE}"
...
read-only v0.agent0.10 verifier.
USAGE
}
