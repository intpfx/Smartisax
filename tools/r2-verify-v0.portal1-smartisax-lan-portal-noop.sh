...
usage() {
  cat <<'USAGE'
Usage:
  tools/r2-verify-v0.portal1-smartisax-lan-portal-noop.sh --offline-image
  tools/r2-verify-v0.portal1-smartisax-lan-portal-noop.sh --read-only
...
--offline-image verifies the candidate without touching a device:
  ...
  - Smartisax declares DevicePortalService and contains the token-gated
    LAN portal /api/pair and /api/status code paths
...
verify_apk_semantics() {
  ...
  grep -q "package: name='com.smartisax.browser' versionCode='6' versionName='0.3.0'" "${WORK_DIR}/smartisax-aapt-badging.txt" \
...
run_offline_image() {
  ...
  local report="${INSPECT_DIR}/verify-${VARIANT}-offline-image-$(date '+%Y%m%d-%H%M%S').txt"
  {
    ...
    echo "result=PASS_OFFLINE_IMAGE_V0PORTAL1_SMARTISAX_LAN_PORTAL_NOOP"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
...
printf "init.svc.bootanim=%s\n" "$(getprop init.svc.bootanim)"'
    ...
    grep -q 'versionCode=6 ' <<<"$package_dump" || die "Smartisax PackageManager did not parse versionCode=6"
    ...
    echo "result=PASS_READ_ONLY_V0PORTAL1_SMARTISAX_LAN_PORTAL_NOOP"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
