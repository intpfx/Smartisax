...
SETTINGS_NOOP_HELD_TAG="${SETTINGS_NOOP_HELD_TAG:-v06}"
...
WORK_DIR="${WORK_DIR:-${ROOT_DIR}/hard-rom/work/${SETTINGS_NOOP_VARIANT}}"
OUT_DIR="${ROOT_DIR}/hard-rom/build"
APK_OUT_DIR="${OUT_DIR}/apk"
RAW_SUPER="${RAW_SUPER:-${WORK_DIR}/super-otatrust-${SETTINGS_NOOP_VARIANT}-exact-current.img}"
SYSTEM_IMG="${SYSTEM_IMG:-${OUT_DIR}/system-otatrust-${SETTINGS_NOOP_VARIANT}.img}"
OUT_SPARSE="${OUT_SPARSE:-${OUT_DIR}/super-otatrust-${SETTINGS_NOOP_VARIANT}-exact-current.sparse.img}"
MANIFEST="${MANIFEST:-${OUT_DIR}/super-otatrust-${SETTINGS_NOOP_VARIANT}-exact-current.SHA256SUMS.txt}"
PROBE_APK="${APK_OUT_DIR}/SettingsSmartisan-certprobe-noop.apk"
PROBE_SIG_REPORT="${APK_OUT_DIR}/SettingsSmartisan-certprobe-noop.signature.txt"
DUMPED_PROBE_APK="${WORK_DIR}/SettingsSmartisan-dumped-from-system.img.apk"
...
Build an offline v0.6 exact-current super candidate from the stable v0.4
baseline. The only ROM change is replacing SettingsSmartisan.apk with a
certificate-readable no-op probe APK that adds an inert asset and preserves the
readable Smartisan Android certificate through the system-partition certs-only
path.
...
Environment:
  ...
  SETTINGS_NOOP_VARIANT=<name>       output variant name; defaults to v0.6-settings-noop
  SETTINGS_NOOP_HELD_TAG=<shorttag>  hidden held-inode tag; defaults to v06
  KEEP_RAW=1                         keep the expanded raw super image
USAGE
}
...
replace_file_in_image() {
  ...
  dir="$(dirname "$dst")"
  base="$(basename "$dst")"
  temp_path="${dir}/.${base}.smartisax-${SETTINGS_NOOP_HELD_TAG}-tmp"
  held_path="${dir}/.${base}.smartisax-${SETTINGS_NOOP_HELD_TAG}-stock-held"
...
build_probe_apk() {
  local tmp
  tmp="$(mktemp -d "/tmp/r2-settings-noop-apk.XXXXXX")"
  trap 'rm -rf "$tmp"' RETURN

  mkdir -p "$APK_OUT_DIR" "${tmp}/assets"
  cp "$STOCK_SETTINGS_APK" "$PROBE_APK"
  printf 'Smartisax SettingsSmartisan cert-probe no-op\n' > "${tmp}/assets/smartisax_cert_probe.txt"
  (
    cd "$tmp"
    zip -q "$PROBE_APK" assets/smartisax_cert_probe.txt
  )

  unzip -l "$PROBE_APK" assets/smartisax_cert_probe.txt >/dev/null
  "$SIGCHECK" "$PROBE_APK" > "$PROBE_SIG_REPORT"
  grep -q "$SMARTISAN_CERT_SHA256" "$PROBE_SIG_REPORT" \
    || die "probe APK does not expose the expected Smartisan Android certificate"
  grep -q '^jarsigner_status=0$' "$PROBE_SIG_REPORT" \
...
rm -f "$RAW_SUPER" "$SYSTEM_IMG" "$OUT_SPARSE" "$MANIFEST" "$PROBE_APK" \
  "$PROBE_SIG_REPORT" "$DUMPED_PROBE_APK" \
...
echo "Building SettingsSmartisan cert-probe no-op APK..."
build_probe_apk
...
replace_file_in_image "$SYSTEM_IMG" "$PROBE_APK" \
  "$SETTINGS_APK_DST" "$SELABEL" \
  "settingssmartisan-noop" >> "${WORK_DIR}/replacements.tsv"

echo "Checking modified system image..."
fsck_image "$SYSTEM_IMG"

echo "Verifying replaced APK after fsck..."
verify_file_in_image_after_fsck "$SYSTEM_IMG" "$PROBE_APK" \
  "$SETTINGS_APK_DST" \
  "settingssmartisan-noop"

probe_hash="$(sha256_one "$PROBE_APK")"
dumped_hash="$(sha256_one "${WORK_DIR}/settingssmartisan-noop-postfsck-dumped.apk")"
...
{
  echo "variant=otatrust-${SETTINGS_NOOP_VARIANT}-exact-current"
  echo "purpose=SettingsSmartisan original-cert-readable no-op replacement probe"
  echo "source_sparse_super=${BASE_SPARSE}"
  echo "settings_noop_variant=${SETTINGS_NOOP_VARIANT}"
  echo "settings_noop_held_tag=${SETTINGS_NOOP_HELD_TAG}"
  ...
  echo "settings_probe_apk=${PROBE_APK}"
  echo "settings_probe_signature_report=${PROBE_SIG_REPORT}"
  echo "settings_probe_inserted_path=${SETTINGS_APK_DST}"
  echo "settings_probe_sha256=${probe_hash}"
  ...
  echo
  shasum -a 256 "$OUT_SPARSE" "$SYSTEM_IMG" "$BASE_SPARSE" "$PROBE_APK" "$STOCK_SETTINGS_APK"
} > "$MANIFEST"

echo "Built: ${OUT_SPARSE}"
echo "System image: ${SYSTEM_IMG}"
echo "Probe APK: ${PROBE_APK}"
echo "Signature report: ${PROBE_SIG_REPORT}"
