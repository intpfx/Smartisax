...
usage() {
  cat <<'USAGE'
Usage:
  tools/r2-verify-v0.40-textboom-ppocr-noop-adapter.sh --offline-image
  tools/r2-verify-v0.40-textboom-ppocr-noop-adapter.sh --read-only

Verifies the v0.40 TextBoom LocalPpOcrApi no-op adapter candidate.
...
print("textboom_changed_payloads=classes2.dex")
PY
}
...
if not (ocr_dir / "CsOcr.smali").exists():
    raise SystemExit("legacy CsOcr unexpectedly removed before real PP-OCR gate")
if not (root / "smali_classes2" / "com" / "intsig" / "csopen").exists():
    raise SystemExit("legacy com.intsig.csopen unexpectedly removed before real PP-OCR gate")

print("textboom_ppocr_noop_adapter_semantics=ok")
PY
}
...
verify_offline_image() {
  ...
  "$JAVA_BIN" -jar "$APKTOOL" if -p "$FRAMEWORK_DIR" "$FW_SMARTISAN" >/dev/null
  {
    ...
    check_manifest_hash "textboom_adapter_apk" "$(manifest_value textboom_adapter_apk)" "textboom_adapter_apk_sha256"
    echo
    ...
    echo
    echo "## APK retention and TextBoom adapter semantics"
    ...
    echo
    echo "result=PASS_OFFLINE_IMAGE_V040_TEXTBOOM_PPOCR_NOOP_ADAPTER"
  } > "$report"
  cat "$report"
  echo "Report: $report"
}
...
verify_read_only_device() {
  ...
  mkdir -p "$INSPECT_DIR" "$WORK_DIR"
  {
    ...
    adb_shell 'ls -ldZ /system/app/TextBoom/lib/arm /system/priv-app/Sidebar /system/app/webview /system/app/SmartisaxShell 2>/dev/null || true'
    ...
    echo
    echo "result=PASS_READ_ONLY_V040_TEXTBOOM_PPOCR_NOOP_ADAPTER"
  } > "$report"
  cat "$report"
  echo "Report: $report"
}
