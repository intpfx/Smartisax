...
SOURCE_VARIANT="v0.36.1-smartisax-shell-debloat-arsc-align"
SOURCE_SPARSE="${SOURCE_SPARSE:-${ROOT_DIR}/hard-rom/build/super-otatrust-v0.36.1-smartisax-shell-debloat-arsc-align.sparse.img}"
SOURCE_SPARSE_SHA256="1dc67299b86a4dde63dc44d2620ce1fe6b6421790bdec082fb12c4c32cc83c03"
SOURCE_SYSTEM_B_IMG="${SOURCE_SYSTEM_B_IMG:-${ROOT_DIR}/hard-rom/build/system-otatrust-v0.36.1-smartisax-shell-debloat-arsc-align.img}"
SOURCE_SYSTEM_B_SHA256="688856c74551f134ef4a2d9bfb103a7dba6e8c1521e5229b2f9c7e455dac6e1b"
...
SOURCE_EXTRACT_DIR="${SOURCE_EXTRACT_DIR:-${WORK_DIR}/source-v0361-retained-slot1}"
SOURCE_RAW="${WORK_DIR}/source-v0361-super.raw.img"
...
PACKAGE_DIR_MTIME_HEX="${PACKAGE_DIR_MTIME_HEX:-0x6a3687c8}"
PACKAGE_DIR_MTIME_NOTE="${PACKAGE_DIR_MTIME_NOTE:-2026-06-20 20:30:00 +0800; invalidates package_cache after TextBoom system-base promotion}"
...
PURPOSE="TextBoom system-base promotion on top of live-proven v0.36.1: installs the live v3.2.2 TextBoom APK byte-for-byte into /system/app/TextBoom without manifest/code/resource edits, preserving the Smartisan v1/JAR signature boundary before later OCR backend work. The existing updated-system /data/app shadow is expected to remain active until a separately confirmed PackageManager cleanup."
...
replace_file_in_image() {
  ...
  dir="$(dirname "$dst")"
  base="$(basename "$dst")"
  temp_path="${dir}/.${base}.smartisax-v037a-tmp"
  held_path="${dir}/.${base}.smartisax-v037a-stock-held"
...
prepare_inputs() {
  ...
  if [ ! -f "${SOURCE_EXTRACT_DIR}/system_a.img" ] || [ ! -f "${SOURCE_EXTRACT_DIR}/system_ext_b.img" ]; then
    echo "Retained partitions are missing; extracting selected slot-1 partitions from v0.36.1 sparse super..."
...

{
  ...
  echo "data_cleanup_gate=TextBoom updated-system /data/app cleanup requires separate explicit user confirmation after boot"
  echo
  ...
  : > "${WORK_DIR}/replaced-system-apks.tsv"

  debugfs_dump "$SYSTEM_B_IMG" "$TEXTBOOM_APK_PATH" "${WORK_DIR}/textboom-stock-before.apk"
  textboom_stock_before_sha256="$(sha256_one "${WORK_DIR}/textboom-stock-before.apk")"
  replace_file_in_image "$SYSTEM_B_IMG" "$TEXTBOOM_APK" "$TEXTBOOM_APK_PATH" "textboom" >> "${WORK_DIR}/replaced-system-apks.tsv"
  set_dir_time "$SYSTEM_B_IMG" "$TEXTBOOM_DIR" "textboom-dir" >> "${WORK_DIR}/package-dir-mtime-bumps.tsv"
  ...
  echo "textboom_stock_before_sha256=${textboom_stock_before_sha256}"
  ...
  python3 "$AVBTOOL" info_image --image "$SYSTEM_B_IMG" > "${WORK_DIR}/system-b-v037a-avb-info.txt"
  grep -q "FEC num roots:         2" "${WORK_DIR}/system-b-v037a-avb-info.txt" || die "system_b lost FEC roots"
  ...
  sparse_hash="$(sha256_one "$OUT_SPARSE")"
  {
    ...
    echo "textboom_data_shadow_expected=yes_until_explicit_cleanup"
    echo "system_free_blocks_before=${system_free_blocks_before}"
    echo "system_free_blocks_after=${system_free_blocks_after}"
    ...
    echo
    echo "# replaced_system_apks"
    cat "${WORK_DIR}/replaced-system-apks.tsv"
    echo
  ...
  echo "result=PASS_BUILD_V037A_TEXTBOOM_LIVE_SYSTEM_BASE"
} 2>&1 | tee "$REPORT"
