...
usage() {
  cat <<'USAGE'
Usage:
  tools/r2-verify-v0.wadb2.1-smartisax-wireless-adb-reflection-pmcache.sh --offline-image
  tools/r2-verify-v0.wadb2.1-smartisax-wireless-adb-reflection-pmcache.sh --read-only
...
verify_apk_semantics() {
  ...
  grep -q "package: name='com.smartisax.browser' versionCode='4' versionName='0.2.2'" "${WORK_DIR}/smartisax-aapt-badging.txt" \
  ...
  grep -R -q 'setAccessible' "${decode_dir}/smali" || die "declared reflection accessibility fix missing from dex"
...
run_offline_image() {
  ...
  local report="${INSPECT_DIR}/verify-${VARIANT}-offline-image-$(date '+%Y%m%d-%H%M%S').txt"
  {
    ...
    echo "result=PASS_OFFLINE_IMAGE_V0WADB21_SMARTISAX_WIRELESS_ADB_REFLECTION_PMCACHE"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
...
printf "init.svc.bootanim=%s\n" "$(getprop init.svc.bootanim)"'
    ...
    grep -q 'versionCode=4 ' <<<"$package_dump" || die "Smartisax PackageManager did not parse versionCode=4"
    ...
    echo "result=PASS_READ_ONLY_V0WADB21_SMARTISAX_WIRELESS_ADB_REFLECTION_PMCACHE"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
