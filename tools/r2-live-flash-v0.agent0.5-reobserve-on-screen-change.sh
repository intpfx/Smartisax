...
RESULT_NAME="PASS_FLASH_V0AGENT05_REOBSERVE_ON_SCREEN_CHANGE"
IMAGE="${ROOT_DIR}/hard-rom/build/super-otatrust-v0.agent0.5-reobserve-on-screen-change.sparse.img"
EXPECTED_IMAGE_SHA256_FILE="${ROOT_DIR}/hard-rom/build/super-otatrust-v0.agent0.5-reobserve-on-screen-change.SHA256SUMS.txt"
CONFIRM_PHRASE="确认刷入 v0.agent0.5-reobserve-on-screen-change B 槽"
INSPECT_DIR="${ROOT_DIR}/hard-rom/inspect/${VARIANT}"
VERIFY_SCRIPT="${ROOT_DIR}/tools/r2-verify-v0.agent0.5-reobserve-on-screen-change.sh"
PREFLIGHT_SCRIPT="${ROOT_DIR}/tools/r2-live-flash-preflight.sh"

usage() {
  cat <<USAGE
Usage:
  tools/r2-live-flash-v0.agent0.5-reobserve-on-screen-change.sh --confirm "${CONFIRM_PHRASE}"
...
flashes super, erases misc, reboots, waits for boot completion, and runs the
	read-only v0.agent0.5 verifier.
USAGE
}
