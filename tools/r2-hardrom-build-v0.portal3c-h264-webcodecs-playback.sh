...
VARIANT="${VARIANT:-v0.portal3c-h264-webcodecs-playback}"
SOURCE_VARIANT="v0.portal3b-h264-http-stream-prototype"
SOURCE_SPARSE="${SOURCE_SPARSE:-${ROOT_DIR}/hard-rom/build/super-otatrust-v0.portal3b-h264-http-stream-prototype.sparse.img}"
SOURCE_SPARSE_SHA256="6ca5e87676adebcfcf1cee26ad13403617bd40a7db4509bf84459adf88b22e07"
SOURCE_SYSTEM_B="${SOURCE_SYSTEM_B:-${ROOT_DIR}/hard-rom/build/system-otatrust-v0.portal3b-h264-http-stream-prototype.img}"
SOURCE_SYSTEM_B_SHA256="c5479f2c19b041fe3ccd150cbf3af1c77d2417fbff34abe419a01935199b8e79"
...
EXPECTED_SERVICES_JAR_SHA256="0b0811858d794f22a4e423f26f4ab27248c25fc4e4b1e6cd95362c0f90b9b97a"
...
PACKAGE_DIR_MTIME_HEX="${PACKAGE_DIR_MTIME_HEX:-0x6a3a4b10}"
PACKAGE_DIR_MTIME_NOTE="${PACKAGE_DIR_MTIME_NOTE:-2026-06-23 18:00:00 +0800; invalidates Smartisax package scan/cache after H.264 WebCodecs playback prototype APK update}"

PURPOSE="Add the first Smartisax Portal H.264 MP4 browser playback prototype on top of live-proven v0.portal3b while retaining the framebuffer services.jar policy, raw H.264 endpoint, WebCodecs diagnostic path, and PNG/input fallback."
RESULT_NAME="PASS_BUILD_V0PORTAL3C_H264_WEBCODECS_PLAYBACK"
...
Builds a system_b-only Smartisax Portal candidate on top of v0.portal3b. It
updates only the Smartisax priv-app APK and privapp whitelist, keeps the narrow
READ_FRAME_BUFFER services.jar policy unchanged, and adds a token-gated
/api/video/mp4 endpoint plus browser-side video-element playback for direct LAN
HTTP. The raw /api/video/h264 endpoint and WebCodecs diagnostic path remain
available, and /api/media/capabilities, PNG screen, and privileged input stay
intact.
It rebuilds system_b AVB/FEC roots=2 and patches that system_b image into the
v0.portal3b sparse super. It does not touch a live device.
USAGE
}
...

{
  ...
  verify_image_file_hash "$SYSTEM_B_IMG" "$SERVICES_JAR_PATH" "$EXPECTED_SERVICES_JAR_SHA256" "services-jar-retained.jar"
  ...
  verify_image_file_hash "$SYSTEM_B_IMG" "$SERVICES_JAR_PATH" "$EXPECTED_SERVICES_JAR_SHA256" "services-jar-retained-after-fsck.jar"
  ...
  system_hash="$(sha256_one "$SYSTEM_B_IMG")"
  {
    ...
    echo "services_jar_retained_sha256=${EXPECTED_SERVICES_JAR_SHA256}"
    echo "services_jar_changed_entries=none"
