...
SIDEBAR_APK_BUILDER="${SIDEBAR_APK_BUILDER:-${ROOT_DIR}/tools/r2-build-sidebar-font-ocr-deleted-apk.sh}"
SKIP_LPDUMP="${SKIP_LPDUMP:-1}"

VARIANT="${VARIANT:-v0.39-sidebar-font-ocr-deleted}"
SOURCE_VARIANT="v0.37b-textboom-live-system-libs-deodex"
SOURCE_SPARSE="${SOURCE_SPARSE:-${ROOT_DIR}/hard-rom/build/super-otatrust-v0.37b-textboom-live-system-libs-deodex.sparse.img}"
SOURCE_SPARSE_SHA256="f8569f4a2a878e7a31ffb54dc352d2a4ccbd304facbd14f3d0fbec7b06a60b04"
SOURCE_SYSTEM_B_IMG="${SOURCE_SYSTEM_B_IMG:-${ROOT_DIR}/hard-rom/build/system-otatrust-v0.37b-textboom-live-system-libs-deodex.img}"
SOURCE_SYSTEM_B_SHA256="5145c6db7da83ecdaa41760d4dacaebc1679751ad76423b2612576050e05017c"
...
SIDEBAR_APK="${SIDEBAR_APK:-${ROOT_DIR}/hard-rom/build/apk/com.smartisanos.sidebar-font-ocr-deleted-v2cert.apk}"
SIDEBAR_APK_SHA256="9a249c3398fa92017294f2b9ff1885d98992404e9cf4a52b848cfce5741ca503"
SIDEBAR_SYSTEM_BEFORE_SHA256="d69e0c7d5960f623795b4c95d1d306f7a2c19b21b22bd6533296943fd4e6772b"
SIDEBAR_PATH="/system/priv-app/Sidebar/Sidebar.apk"
SIDEBAR_DIR="/system/priv-app/Sidebar"
...
PACKAGE_DIR_MTIME_NOTE="${PACKAGE_DIR_MTIME_NOTE:-generated at build time to invalidate PackageCacher for Sidebar}"

TEXTBOOM_APK="/system/app/TextBoom/TextBoom.apk"
TEXTBOOM_SHA256="52df3deb5315baf41b9f5476a122ce9782fa58f74076d1d4a9c060c9c506873c"
...
PURPOSE="Code-delete retired Sidebar/One Step font OCR on top of live-stable v0.37b, preserving TextBoom, M150 WebView, Smartisax shell, v0.29 Sidebar topbar blank-slot behavior, and system_b AVB/FEC."
...
verify_inputs() {
  ...
  if [ ! -f "$SIDEBAR_APK" ]; then
    "$SIDEBAR_APK_BUILDER" >/dev/null
  fi
  require_hash "$SIDEBAR_APK" "$SIDEBAR_APK_SHA256"
...

{
  ...
  verify_apk_hash "$SYSTEM_B_IMG" "$SIDEBAR_PATH" "$SIDEBAR_SYSTEM_BEFORE_SHA256" "sidebar-before"
  verify_apk_hash "$SYSTEM_B_IMG" "$TEXTBOOM_APK" "$TEXTBOOM_SHA256" "textboom-retained"
  ...
  replace_file_in_image "$SYSTEM_B_IMG" "$SIDEBAR_APK" "$SIDEBAR_PATH" \
    "sidebar-font-ocr-deleted" >> "${WORK_DIR}/replacements.tsv"

  : > "${WORK_DIR}/package-dir-mtime-bumps.tsv"
  bump_package_dir_time "$SYSTEM_B_IMG" "$SIDEBAR_DIR" "sidebar-dir" >> "${WORK_DIR}/package-dir-mtime-bumps.tsv"
  bump_package_dir_time "$SYSTEM_B_IMG" "/system/priv-app" "system-priv-app" >> "${WORK_DIR}/package-dir-mtime-bumps.tsv"

  fsck_rw "$SYSTEM_B_IMG"
  fsck_ro "$SYSTEM_B_IMG"
  verify_apk_hash "$SYSTEM_B_IMG" "$SIDEBAR_PATH" "$SIDEBAR_APK_SHA256" "sidebar-after"
  verify_apk_hash "$SYSTEM_B_IMG" "$TEXTBOOM_APK" "$TEXTBOOM_SHA256" "textboom-after"
  ...
  python3 "$AVBTOOL" info_image --image "$SYSTEM_B_IMG" > "${WORK_DIR}/system-b-v039-avb-info.txt"
  grep -q "FEC num roots:         2" "${WORK_DIR}/system-b-v039-avb-info.txt" || die "system_b lost FEC roots"
...

  {
    ...
    echo "sidebar_apk=${SIDEBAR_APK}"
    echo "sidebar_apk_sha256=${sidebar_hash}"
    echo "sidebar_system_path=${SIDEBAR_PATH}"
    echo "sidebar_system_before_sha256=${SIDEBAR_SYSTEM_BEFORE_SHA256}"
    echo "sidebar_font_ocr=code_deleted"
    echo "sidebar_boom_font_activity=removed"
    echo "sidebar_action_boom_font=removed"
    echo "sidebar_open_font_classes=removed"
    echo "sidebar_intsig_sdk=removed"
    echo "sidebar_identify_font_view=removed"
    echo "sidebar_tool_button_type1_mapping=removed"
    echo "textboom_apk_sha256=${TEXTBOOM_SHA256}"
    ...
    echo
    shasum -a 256 "$OUT_SPARSE" "$SYSTEM_B_IMG" "$SOURCE_PRODUCT_B_IMG" "$SIDEBAR_APK"
  } > "$MANIFEST"
  cp "$MANIFEST" "${OUT_SPARSE}.SHA256SUMS.txt"
  echo "manifest=${MANIFEST}"
  echo "result=PASS_BUILD_V039_SIDEBAR_FONT_OCR_DELETED"
} 2>&1 | tee "$REPORT"
