...
BASE_SERVICES="${ROOT_DIR}/hard-rom/build/framework/services-kg1-smartisax-skip-keyguard.jar"
BASE_SERVICES_JAR_SHA256="0f8991d4f9d7f0bf65407d62c180a8e98852135584f05cda5a57cba955fae9b6"
SERVICES_JAR_CANDIDATE_SHA256="59e1174901e914684a15e1bb22f82bc5683e23c91ec9a74123f7367826ab8ce2"
SENTINEL="__smartisax_current_wifi__"

usage() {
  cat <<'USAGE'
Usage:
  tools/r2-verify-v0.wadb2-smartisax-wireless-adb-current-wifi.sh --offline-image
  tools/r2-verify-v0.wadb2-smartisax-wireless-adb-current-wifi.sh --read-only
...
verify_apk_semantics() {
  ...
  grep -q "package: name='com.smartisax.browser' versionCode='3' versionName='0.2.1'" "${WORK_DIR}/smartisax-aapt-badging.txt" \
...
run_offline_image() {
  ...
  local report="${INSPECT_DIR}/verify-${VARIANT}-offline-image-$(date '+%Y%m%d-%H%M%S').txt"
  {
    ...
    echo "result=PASS_OFFLINE_IMAGE_V0WADB2_SMARTISAX_WIRELESS_ADB_CURRENT_WIFI"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
...
printf "init.svc.bootanim=%s\n" "$(getprop init.svc.bootanim)"'
    ...
    echo "result=PASS_READ_ONLY_V0WADB2_SMARTISAX_WIRELESS_ADB_CURRENT_WIFI"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
