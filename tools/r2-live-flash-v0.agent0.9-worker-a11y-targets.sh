...
RESULT_NAME="PASS_FLASH_V0AGENT09_WORKER_A11Y_TARGETS"
IMAGE="${ROOT_DIR}/hard-rom/build/super-otatrust-v0.agent0.9-worker-a11y-targets.sparse.img"
EXPECTED_IMAGE_SHA256_FILE="${ROOT_DIR}/hard-rom/build/super-otatrust-v0.agent0.9-worker-a11y-targets.SHA256SUMS.txt"
CONFIRM_PHRASE="确认刷入 v0.agent0.9-worker-a11y-targets B 槽"
INSPECT_DIR="${ROOT_DIR}/hard-rom/inspect/${VARIANT}"
VERIFY_SCRIPT="${ROOT_DIR}/tools/r2-verify-v0.agent0.9-worker-a11y-targets.sh"
PREFLIGHT_SCRIPT="${ROOT_DIR}/tools/r2-live-flash-preflight.sh"

usage() {
  cat <<USAGE
Usage:
  tools/r2-live-flash-v0.agent0.9-worker-a11y-targets.sh --confirm "${CONFIRM_PHRASE}"
...
read-only v0.agent0.9 verifier.
USAGE
}
