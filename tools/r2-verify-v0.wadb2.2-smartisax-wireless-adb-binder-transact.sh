...
usage() {
  cat <<'USAGE'
Usage:
  tools/r2-verify-v0.wadb2.2-smartisax-wireless-adb-binder-transact.sh --offline-image
  tools/r2-verify-v0.wadb2.2-smartisax-wireless-adb-binder-transact.sh --read-only
...
verify_apk_semantics() {
  ...
  grep -q "package: name='com.smartisax.browser' versionCode='5' versionName='0.2.3'" "${WORK_DIR}/smartisax-aapt-badging.txt" \
...
run_offline_image() {
  ...
  local report="${INSPECT_DIR}/verify-${VARIANT}-offline-image-$(date '+%Y%m%d-%H%M%S').txt"
  {
    ...
    echo "result=PASS_OFFLINE_IMAGE_V0WADB22_SMARTISAX_WIRELESS_ADB_BINDER_TRANSACT"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
...
printf "init.svc.bootanim=%s\n" "$(getprop init.svc.bootanim)"'
    ...
    grep -q 'versionCode=5 ' <<<"$package_dump" || die "Smartisax PackageManager did not parse versionCode=5"
    ...
    echo "result=PASS_READ_ONLY_V0WADB22_SMARTISAX_WIRELESS_ADB_BINDER_TRANSACT"
  } 2>&1 | tee "$report"
  echo "Report: $report"
}
