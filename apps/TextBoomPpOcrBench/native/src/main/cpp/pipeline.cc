...
cv::Mat GetRotateCropImage(cv::Mat srcimage,
  ...
  int right =
      std::min(image.cols, int(*std::max_element(x_collect, x_collect + 4)));
  int top = std::max(0, int(*std::min_element(y_collect, y_collect + 4)));
  int bottom =
      std::min(image.rows, int(*std::max_element(y_collect, y_collect + 4)));
...
int BoxTop(const std::vector<std::vector<int>> &box) {
  int top = box.empty() || box[0].size() < 2 ? 0 : box[0][1];
...
int BoxLeft(const std::vector<std::vector<int>> &box) {
  int left = box.empty() || box[0].empty() ? 0 : box[0][0];
  for (const auto &point : box) {
    if (!point.empty()) {
...
void SortBoxesReadingOrder(std::vector<std::vector<std::vector<int>>> *boxes) {
  std::sort(boxes->begin(), boxes->end(), [](const auto &lhs, const auto &rhs) {
    int lhs_top = BoxTop(lhs);
    int rhs_top = BoxTop(rhs);
    if (std::abs(lhs_top - rhs_top) > 12) {
      return lhs_top < rhs_top;
    }
    return BoxLeft(lhs) < BoxLeft(rhs);
  });
}
...
OcrRunResult Pipeline::RunImage(const std::string &imagePath) {
  OcrRunResult result;
  auto total_start = GetCurrentTime();

  cv::Mat srcimg = cv::imread(imagePath, cv::IMREAD_COLOR);
  if (srcimg.empty()) {
    result.status = "IMAGE_DECODE_FAILED";
    result.error = "cv::imread returned empty image";
    result.total_ms = GetElapsedTime(total_start);
    return result;
  }
  result.image_width = srcimg.cols;
  result.image_height = srcimg.rows;

  try {
    auto det_start = GetCurrentTime();
    auto boxes =
        detPredictor_->Predict(srcimg, Config_, nullptr, nullptr, nullptr);
    SortBoxesReadingOrder(&boxes);
    result.det_ms = GetElapsedTime(det_start);

    int use_direction_classify = int(Config_["use_direction_classify"]);
    auto rec_start = GetCurrentTime();
    for (const auto &box : boxes) {
      cv::Mat crop_img = GetRotateCropImage(srcimg, box);
      if (crop_img.empty()) {
        continue;
      }
      ...
      auto rec =
          recPredictor_->Predict(crop_img, nullptr, nullptr, nullptr,
                                 charactor_dict_);
      OcrLine line;
      line.text = rec.first;
      line.score = rec.second;
      line.box = box;
      result.lines.push_back(line);
    }
    result.rec_ms = GetElapsedTime(rec_start);
    result.total_ms = GetElapsedTime(total_start);
    result.ok = true;
    result.status = "PP_OCR_READY";
  } catch (const std::exception &error) {
    result.status = "PP_OCR_ERROR";
...
bool Pipeline::Process_val(int inTextureId, int outTextureId, int textureWidth,
  ...
  std::vector<std::string> rec_text;
  std::vector<float> rec_text_score;
  LOGD("debug===boxes: %zu", boxes.size());
