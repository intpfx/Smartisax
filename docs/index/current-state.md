...
v0.agent0-vision-loop: previous live flashed/read-only Agent MVP line on top of live/read-only v0.portal6g. It updates Smartisax to v0.7.0/versionCode 51, adds an on-device SmartisaxAgentRuntime with max 5 observe-plan-execute steps, MiMo V2.5 vision-first planner, DeepSeek v4 flash text/status fallback, mock provider, shared JPEG/Base64 SurfaceControl screenshot observation, local SharedPreferences API-key storage, Shell Agent UI, and token-gated read-only `/api/agent/status`. The allowed action surface is tap, swipe, BACK/HOME key, wait, finish, and ask_user; shell/root/ADB/fastboot/flash/erase/data cleanup remain excluded, and `/api/input` remains absent. APK hash is `c4731c66a6d4413317b7f96b03c3e7a83f91fd3f1afefad0974d67033e879679`, system_b hash is `93ca6cedd9676bd53facb44c686f6b02b6ac8f9d933e66ca8896175390e49b82`, sparse super hash is `c4b757bf09edd043c932f76e978aeefe1a426bf57e5c4f8f078084a60dcdbb3f`, build result is `PASS_BUILD_V0AGENT0_VISION_LOOP`, offline result is `PASS_OFFLINE_IMAGE_V0AGENT0_VISION_LOOP` with `PASS_AGENT0_OFFLINE_TESTS` and `agent0_extra_offline_checks=ok`, preflight evidence is `hard-rom/inspect/v0.agent0-vision-loop/preflight-v0.agent0-vision-loop-20260630-163820.txt`, flash result is `PASS_FLASH_V0AGENT0_VISION_LOOP`, and read-only live result is `PASS_READ_ONLY_V0AGENT0_VISION_LOOP`. Post-flash proof: boot_completed=1, slot `_b`, bootanim stopped, verified boot orange, root available, SELinux Enforcing, Smartisax Shell focused, display ON/Awake, `/api/status` and `/api/agent/status` token-gated contracts pass, and `/api/screen.png` returns a 1080x2340 PNG. MiMo safe-finish live smoke passed: manually-started `mimo_v25_vision` used a non-persisted 1080x2340 screenshot scaled to 720x1560 JPEG, returned one-step `finish` with confidence 1, and ended in state `complete`. The first constrained Settings tap smoke is diagnostic FAIL: MiMo identified the gray gear Settings icon but returned y coordinates too high for the visible icon center, the runtime injected valid-but-wrong taps, then accepted `finish` while the foreground app remained `com.smartisax.browser/.ShellActivity`. Evidence: `hard-rom/inspect/v0.agent0-vision-loop/flash-v0.agent0-vision-loop-20260630-164247.txt`, `hard-rom/inspect/v0.agent0-vision-loop/verify-v0.agent0-vision-loop-device-read-only-20260630-164929.txt`, `hard-rom/inspect/v0.agent0-vision-loop/display-window-state-after-flash-20260630-165332.txt`, `hard-rom/inspect/v0.agent0-vision-loop/post-flash-agent0-smoke-20260630-165307/report.txt`, `hard-rom/inspect/v0.agent0-vision-loop/mimo-smoke/mimo-vision-task-20260630-171243/report.txt`, and `hard-rom/inspect/v0.agent0-vision-loop/mimo-smoke/settings-click-agent2-20260630-180500/`.

v0.agent0.1-vision-guard: previous live flashed/read-only Agent guard line on top of the live/read-only v0.agent0 system image. It updates Smartisax to v0.7.1/versionCode 52, preserves the same narrow action surface and no remote HTTP Agent start/control, and repairs the first Settings tap false-complete by adding post-action screenshot observations, non-persisted screenshot fingerprints, `postActionCheck` transcript/status fields, `coordinate_edge_guard` for top/bottom edge taps, `repeated_tap_no_screen_change` pause, and `finish_requires_verified_screen_change` after UI actions. APK hash is `3de1cd9605310d4da2316c70066296f796352a0bb09f29ac641f99e3f18ae40e`, system_b hash is `6afaa75773178b2ee613f435817bed4542ada4880e5721f9ff90345de308451f`, and sparse hash is `4456d0b9e3d2b05a05bebfca08424a4ee4dd5f61d3240a83a93b2a7dfb9b6458`. Build result is `PASS_BUILD_V0AGENT01_VISION_GUARD`; offline result is `PASS_OFFLINE_IMAGE_V0AGENT01_VISION_GUARD` with `PASS_AGENT0_OFFLINE_TESTS` and `agent01_extra_offline_checks=ok`; flash result is `PASS_FLASH_V0AGENT01_VISION_GUARD`; read-only live result is `PASS_READ_ONLY_V0AGENT01_VISION_GUARD`. Post-flash proof: boot_completed=1, slot `_b`, bootanim stopped, verified boot orange, root available, SELinux Enforcing, Smartisax Shell focused, display awake/on, keyguard not showing, device APK hash matches, and system libwebrtc hashes match. Reusing the old local Portal token after reboot timed out on `/api/status`, `/api/agent/status`, and `/api/screen.png`, consistent with explicit-opt-in Portal not autostarting; rerun Portal regression after enabling/pairing Portal. A post-flash Settings tap task is diagnostic FAIL for capability but PASS for guard behavior: final state `paused`, step `5/5`, last `max_steps_reached`, ShellActivity still focused, and the transcript shows per-step actions/post-checks instead of a false `complete`. Evidence: `hard-rom/inspect/v0.agent0.1-vision-guard/build-v0.agent0.1-vision-guard-20260630-183031.txt`, `hard-rom/inspect/v0.agent0.1-vision-guard/verify-v0.agent0.1-vision-guard-offline-image-20260630-183352.txt`, `hard-rom/inspect/v0.agent0.1-vision-guard/flash-v0.agent0.1-vision-guard-20260630-185030.txt`, `hard-rom/inspect/v0.agent0.1-vision-guard/boot-wait-v0.agent0.1-vision-guard-20260630-185030.txt`, `hard-rom/inspect/v0.agent0.1-vision-guard/verify-v0.agent0.1-vision-guard-device-read-only-20260630-185541.txt`, `hard-rom/inspect/v0.agent0.1-vision-guard/post-flash-portal-status-20260630-185646/report.txt`, and `hard-rom/inspect/v0.agent0.1-vision-guard/post-flash-settings-task-20260630-190005/report.txt`.

v0.agent0.2-one-step: previous live flashed/read-only Agent One Step line on top of v0.agent0.1. It updates Smartisax to v0.7.2/versionCode 53 and teaches the model a narrow `one_step` action with `operation=enter|exit` for Smartisan One Step mode. Runtime execution mirrors Smartisan's framework `SidebarHelper` client path through `IWindowManager.transact(2001)` with mode `2` for right One Step/sidebar enter and mode `-1` for exit, then falls back to a controlled right-edge swipe for enter or BACK for exit if the requested One Step state is not reached. APK hash is `6bbdf9022af6f4ba1d66a5cb67ae738ebda2be39f38d8c07cc80856681ea11c0`, system_b hash is `c8ffc8592067eb72ce303d3bb1e559e8553a342efca8ff6873ee98f29091c4a8`, and sparse hash is `b30c3d6a1ed6ba0c9f31ae722b77c869810be734f73db8131d3b6f5e63efc2a9`. Build result is `PASS_BUILD_V0AGENT02_ONE_STEP`; offline result is `PASS_OFFLINE_IMAGE_V0AGENT02_ONE_STEP` with `PASS_AGENT0_OFFLINE_TESTS` and `agent02_extra_offline_checks=ok`; flash result is `PASS_FLASH_V0AGENT02_ONE_STEP`; read-only result is `PASS_READ_ONLY_V0AGENT02_ONE_STEP`. Post-flash proof: boot_completed=1, slot `_b`, bootanim stopped, verified boot orange, root available, SELinux Enforcing, device Smartisax APK hash matches, and privapp permissions are unchanged. The first safe One Step smoke is diagnostic FAIL but useful: MiMo emitted `one_step(enter)` then `one_step(exit)`, but the v0.agent0.2 controller sampled state too soon after the first `IWindowManager.transact(2001)` while SidebarService async binding had not finished, so the action result stayed hidden; finish gate paused with `finish_requires_verified_screen_change` instead of false-completing. Manual diagnostics then proved `service call window 2001 i32 2 i32 0` can set `side_bar_zoom_type=2` and `sidebar_switch_status=1`, and `i32 -1` exits to `-1/0`, confirming the route is correct and the fix belongs in the Agent wait/guard layer. Evidence: `hard-rom/inspect/v0.agent0.2-one-step/flash-v0.agent0.2-one-step-20260630-193243.txt`, `hard-rom/inspect/v0.agent0.2-one-step/verify-v0.agent0.2-one-step-device-read-only-20260630-193754.txt`, and `hard-rom/inspect/v0.agent0.2-one-step/one-step-smoke-20260630-193820/`.

v0.agent0.3-one-step-bind-wait: previous live flashed/read-only Agent repair line on top of v0.agent0.2. It updates Smartisax to v0.7.3/versionCode 54 and repairs One Step execution by waiting up to 2800ms after the first `IWindowManager.transact(2001)` for async SidebarService binding, retrying programmatic entry once, and pausing immediately with `one_step_state_guard` / `one_step_enter_not_visible` if the requested One Step state is still not reached. APK hash is `901cf3ae91f1d02e330dddd67890aaafa71cac815973cbc318df4cedcab493fc`, system_b hash is `574aace6c53aa704144a572040de2510c1ff8ba88119a0966f38a4799c00f942`, and sparse hash is `afc2d90ceee5e59036c4f9dd4ae7e4096dd1284f5614f4e6afa5c7ad3c8ae056`. Build result is `PASS_BUILD_V0AGENT03_ONE_STEP_BIND_WAIT`; offline result is `PASS_OFFLINE_IMAGE_V0AGENT03_ONE_STEP_BIND_WAIT` with `PASS_AGENT0_OFFLINE_TESTS` and `agent03_extra_offline_checks=ok`; flash result is `PASS_FLASH_V0AGENT03_ONE_STEP_BIND_WAIT`; read-only result is `PASS_READ_ONLY_V0AGENT03_ONE_STEP_BIND_WAIT`. Post-flash proof: boot_completed=1, slot `_b`, bootanim stopped, verified boot orange, root available, SELinux Enforcing, device Smartisax APK hash matches, and system libwebrtc hashes match. The clean One Step smoke passes after normalizing stale statusbar/keyguard residue: MiMo emitted `one_step(enter)`, `one_step(exit)`, then `finish`; the run ended `complete`, step `3/5`, One Step hidden, with transcript results `one-step: enter visible right` and `one-step: exit hidden none`. The broader Settings App task is still diagnostic FAIL but guard-correct: final state `paused`, step `5/5`, last `max_steps_reached`, ShellActivity still focused, and the transcript shows repeated `key(HOME)` actions rather than a false complete. Diagnosis: SmartisaxShell can be the HOME target, so Settings-open planning must route through One Step's top app strip instead of looping on HOME. Evidence: `hard-rom/inspect/v0.agent0.3-one-step-bind-wait/build-v0.agent0.3-one-step-bind-wait-20260630-195448.txt`, `hard-rom/inspect/v0.agent0.3-one-step-bind-wait/verify-v0.agent0.3-one-step-bind-wait-offline-image-20260630-195752.txt`, `hard-rom/inspect/v0.agent0.3-one-step-bind-wait/flash-v0.agent0.3-one-step-bind-wait-20260630-200648.txt`, `hard-rom/inspect/v0.agent0.3-one-step-bind-wait/verify-v0.agent0.3-one-step-bind-wait-device-read-only-20260630-201158.txt`, `hard-rom/inspect/v0.agent0.3-one-step-bind-wait/one-step-smoke-20260630-201232/`, and `hard-rom/inspect/v0.agent0.3-one-step-bind-wait/settings-app-task-20260630-203440/`.

v0.agent0.4-home-onestep-settings-guard: previous live flashed/read-only Agent repair line on top of live/read-only v0.agent0.3. It updates Smartisax to v0.7.4/versionCode 55 and repairs the v0.agent0.3 Settings-open failure by telling providers that SmartisaxShell may itself be the HOME target, and that Settings goals from SmartisaxShell should use `one_step enter` and tap the gear-shaped Settings icon in the One Step top app strip instead of repeatedly pressing HOME. The runtime also adds a repeated key no-screen-change pause with reason `repeated_key_no_screen_change`. APK hash is `d200a807710af02604038050a2d6f460051e19a34e18a5b334f7b65ec4cabd6a`, system_b hash is `bf4c989ecd162fbcdca4d4122fc376d0444031f0def4ce658b35cad8022d8873`, and sparse hash is `c3aa40da9294a3db7e28aa81e91bfd244b717d11a0c96fd71b1b1b28d2107fc5`. Build result is `PASS_BUILD_V0AGENT04_HOME_ONESTEP_SETTINGS_GUARD`; offline result is `PASS_OFFLINE_IMAGE_V0AGENT04_HOME_ONESTEP_SETTINGS_GUARD` with `PASS_AGENT0_OFFLINE_TESTS` and `agent04_extra_offline_checks=ok`; flash result is `PASS_FLASH_V0AGENT04_HOME_ONESTEP_SETTINGS_GUARD`; read-only result is `PASS_READ_ONLY_V0AGENT04_HOME_ONESTEP_SETTINGS_GUARD`. Post-flash Settings diagnostics: the first run paused at `one_step_enter_not_visible` because stale Sidebar/Zoom keyguard state reported `request zoom failed cause keyguard showing`; after UI normalization, `one_step(enter)` succeeds and One Step is visible, but the next MiMo action returns `tap(9275,250)`, which is blocked by `coordinate_edge_guard`. This proves Settings-open planning no longer loops on HOME, and the next repair belongs in runtime reobserve/replan after material screen changes. Evidence: `hard-rom/inspect/v0.agent0.4-home-onestep-settings-guard/build-v0.agent0.4-home-onestep-settings-guard-20260630-210029.txt`, `hard-rom/inspect/v0.agent0.4-home-onestep-settings-guard/verify-v0.agent0.4-home-onestep-settings-guard-offline-image-20260630-210333.txt`, `hard-rom/inspect/v0.agent0.4-home-onestep-settings-guard/flash-v0.agent0.4-home-onestep-settings-guard-20260630-211650.txt`, `hard-rom/inspect/v0.agent0.4-home-onestep-settings-guard/verify-v0.agent0.4-home-onestep-settings-guard-device-read-only-20260630-212201.txt`, and `hard-rom/inspect/v0.agent0.4-home-onestep-settings-guard/settings-app-task-20260630-212239/`.

v0.agent0.5-reobserve-on-screen-change: previous live Agent repair line on top of live/read-only v0.agent0.4. It updates Smartisax to v0.7.5/versionCode 56 and moves stale-coordinate recovery from prompt-only guidance into runtime material screen-change detection. `SmartisaxScreenCapture.AgentObservation` now includes a 12x12 visual signature plus `visualDistance` / `changedCells` metadata; the runtime uses `screen_freshness_guard` to skip stale actions when the screen changed between planning and execution, and uses `coordinate_guard_after_screen_change_reobserve` / `skipped_coordinate_guard_reobserve` to reobserve after a large post-action screen transition before pausing on an edge coordinate. Reobserve skips are bounded at two. APK hash is `6e1aba0b426957bc88b561dfc4cc40677a8f42c5fde1b908f9896a5a1879e45f`, system_b hash is `90622eefcf994ebbf5f58aeca9cb4f7bd67b67e9782b7a24d983f8f5de16e8e1`, and sparse hash is `09c157326d12dd95b5b0aaaa7783daebb0292e46cd1fb064923cd33654f17f47`. Build result is `PASS_BUILD_V0AGENT05_REOBSERVE_ON_SCREEN_CHANGE`; offline result is `PASS_OFFLINE_IMAGE_V0AGENT05_REOBSERVE_ON_SCREEN_CHANGE` with `PASS_AGENT0_OFFLINE_TESTS` and `agent05_extra_offline_checks=ok`; flash result is `PASS_FLASH_V0AGENT05_REOBSERVE_ON_SCREEN_CHANGE`; read-only result is `PASS_READ_ONLY_V0AGENT05_REOBSERVE_ON_SCREEN_CHANGE`. Post-flash proof: boot_completed=1, slot `_b`, bootanim stopped, verified boot orange, root available, SELinux Enforcing, current focus `com.smartisax.browser/.ShellActivity`, `isKeyguardShowing=false`, device APK hash matches, and system libwebrtc hashes match. Evidence: `hard-rom/inspect/v0.agent0.5-reobserve-on-screen-change/build-v0.agent0.5-reobserve-on-screen-change-20260630-214854.txt`, `hard-rom/inspect/v0.agent0.5-reobserve-on-screen-change/verify-v0.agent0.5-reobserve-on-screen-change-offline-image-20260630-215157.txt`, `hard-rom/inspect/v0.agent0.5-reobserve-on-screen-change/flash-v0.agent0.5-reobserve-on-screen-change-20260630-220422.txt`, `hard-rom/inspect/v0.agent0.5-reobserve-on-screen-change/verify-v0.agent0.5-reobserve-on-screen-change-device-read-only-20260630-220932.txt`, and `hard-rom/inspect/v0.agent0.5-reobserve-on-screen-change/post-flash-focus-v0.agent0.5-reobserve-on-screen-change-20260630-221006.txt`.

v0.agent0.5 Settings-open diagnostic: still FAIL for opening Settings, but guard-correct. MiMo now taps the visible One Step top-strip Settings gear at non-edge coordinates (`tap(9100,1700)` and `tap(9250,1700)`), the runtime pauses with `repeated_tap_no_screen_change`, and a manual `input tap 982 398` probe on the apparent gear area also leaves focus on `com.smartisax.browser/.ShellActivity`. Treat the next repair as a One Step top app strip launch/click mechanism problem, or as a deliberate narrow Settings intent action design, not as a prompt-only coordinate wording problem. Evidence: `hard-rom/inspect/v0.agent0.5-reobserve-on-screen-change/settings-app-task-20260630-221422/`.

v0.agent0.6-accessibility-tree: previous live flashed/read-only Agent repair line on top of live/read-only v0.agent0.5. It updates Smartisax to v0.7.6/versionCode 57, registers `SmartisaxAccessibilityService`, enables it through `Settings.Secure.ENABLED_ACCESSIBILITY_SERVICES`, sends a bounded compact Accessibility tree in each Agent step prompt, and adds a narrow `click_node` action that re-queries the current tree and executes `AccessibilityNodeInfo.performAction(ACTION_CLICK)`. It also pauses repeated same-node no-change loops with `repeated_node_no_screen_change`. It preserves the existing no shell/root/ADB/fastboot/erase/delete Agent boundary and keeps `/api/input` absent. APK hash is `a0109ab1ceaea4c6039eb43227c5c601edb5464bda52f2a7e889c39964387389`, system_b hash is `143cc0674a8d451d76d63b4c9d61a8bda857310d5d26a6eb84f0ce19ff1269b9`, and sparse hash is `8f9c050815555ca38c0c7aa35fb3ed88497f4680e57ad8e15a3d75072c298fa7`. Build result is `PASS_BUILD_V0AGENT06_ACCESSIBILITY_TREE`; offline result is `PASS_OFFLINE_IMAGE_V0AGENT06_ACCESSIBILITY_TREE` with `PASS_AGENT0_OFFLINE_TESTS` and `agent06_extra_offline_checks=ok`; flash result is `PASS_FLASH_V0AGENT06_ACCESSIBILITY_TREE`; read-only result is `PASS_READ_ONLY_V0AGENT06_ACCESSIBILITY_TREE`. Post-flash proof: boot_completed=1, slot `_b`, bootanim stopped, verified boot orange, root available, SELinux Enforcing, device APK hash matches, `accessibility_enabled=1`, enabled service is `com.smartisax.browser/com.smartisax.browser.SmartisaxAccessibilityService`, and `dumpsys accessibility` shows the Smartisax service bound with the Smartisax window active/focused. The live Settings task diagnostic is blocked but useful: the MiMo task stayed at `running step=0/5` because the device had no usable Internet/DNS path, and a manual One Step probe showed the top strip/Settings gear visually present but not exposed as a Settings `click_node` in the active Accessibility tree; `dumpsys accessibility` only showed separate unknown overlay windows and `dumpsys window` confirmed visible `com.smartisanos.sidebar` top/right surfaces. That diagnostic motivated v0.agent0.7's provider preflight plus full `getWindows()` Accessibility collection. Evidence: `hard-rom/inspect/v0.agent0.6-accessibility-tree/build-v0.agent0.6-accessibility-tree-20260701-000041.txt`, `hard-rom/inspect/v0.agent0.6-accessibility-tree/verify-v0.agent0.6-accessibility-tree-offline-image-20260701-000415.txt`, `hard-rom/inspect/v0.agent0.6-accessibility-tree/flash-v0.agent0.6-accessibility-tree-20260701-154742.txt`, `hard-rom/inspect/v0.agent0.6-accessibility-tree/verify-v0.agent0.6-accessibility-tree-device-read-only-20260701-155253.txt`, `hard-rom/inspect/v0.agent0.6-accessibility-tree/post-flash-accessibility-v0.agent0.6-accessibility-tree-20260701-155309.txt`, and `hard-rom/inspect/v0.agent0.6-accessibility-tree/settings-task-20260701-160601/report.md`.

v0.agent0.7-window-preflight: previous live flashed/read-only Agent repair line on top of live/read-only v0.agent0.6. It updates Smartisax to v0.7.7/versionCode 58, extends `SmartisaxAccessibilityService` observations from active-root-only to active root plus `AccessibilityService.getWindows()` interactive-window roots, extends `click_node` lookup across those roots, and adds visible provider planning/network/timeout transcript guards so provider failures pause with normalized reasons instead of leaving the UI at `running step=0/5`. APK hash is `68b9cc0da7fd8e8d03ac4606fb9dd46329993af05b92896890d787db5317a74b`, system_b hash is `4c1cee130f776f3fe83340dbef7592cc56ea4e37446aefa548f5cf3f378bc892`, and sparse hash is `d16518056abea641cf51e8d944eb517a00dfdbd3d4ba7ef44a5cbad30400c7cc`. Build result is `PASS_BUILD_V0AGENT07_WINDOW_PREFLIGHT`; offline result is `PASS_OFFLINE_IMAGE_V0AGENT07_WINDOW_PREFLIGHT` with `PASS_AGENT0_OFFLINE_TESTS` and `agent07_extra_offline_checks=ok`. The first confirmed flash attempt was blocked before mutation because the R2 was not visible over ADB or fastboot. After USB reconnect, the confirmed retry flashed sparse `super` chunks 1/9 through 9/9 successfully, erased `misc`, rebooted, and passed read-only verification. Results: `PASS_FLASH_V0AGENT07_WINDOW_PREFLIGHT` and `PASS_READ_ONLY_V0AGENT07_WINDOW_PREFLIGHT`. Post-flash proof: boot_completed=1, slot `_b`, bootanim stopped, verified boot orange, root available, SELinux Enforcing, package versionName `0.7.7`/versionCode `58`, device APK hash matches, Smartisax Shell focused, `isKeyguardShowing=false`, and enabled Accessibility services include `com.smartisax.browser/com.smartisax.browser.SmartisaxAccessibilityService`. The live Settings/getWindows diagnostic proved `getWindows()` can expose visible One Step overlay roots after UI normalization, but those roots still lacked a usable dynamic Settings node, motivating v0.agent0.8. Evidence: `hard-rom/inspect/v0.agent0.7-window-preflight/build-v0.agent0.7-window-preflight-20260701-162941.txt`, `hard-rom/inspect/v0.agent0.7-window-preflight/verify-v0.agent0.7-window-preflight-offline-image-20260701-163244.txt`, `hard-rom/inspect/v0.agent0.7-window-preflight/preflight-v0.agent0.7-window-preflight-20260701-163520.txt`, `hard-rom/inspect/v0.agent0.7-window-preflight/flash-v0.agent0.7-window-preflight-20260701-163916.txt`, `hard-rom/inspect/v0.agent0.7-window-preflight/flash-v0.agent0.7-window-preflight-20260701-165128.txt`, `hard-rom/inspect/v0.agent0.7-window-preflight/boot-wait-v0.agent0.7-window-preflight-20260701-165128.txt`, `hard-rom/inspect/v0.agent0.7-window-preflight/verify-v0.agent0.7-window-preflight-device-read-only-20260701-165639.txt`, `hard-rom/inspect/v0.agent0.7-window-preflight/post-flash-focus-accessibility-v0.agent0.7-window-preflight-20260701-165715.txt`, and `hard-rom/inspect/v0.agent0.7-window-preflight/settings-task-20260701-170535/retry-after-replug/`.

v0.agent0.8-onestep-a11y-nodes: previous live flashed/read-only Agent line on top of live/read-only v0.agent0.7. It updates Smartisax to v0.7.8/versionCode 59, adds a One Step enter visible-state recovery path (`HOME -> exit -> enter`) before touch fallback, and patches the current Sidebar APK with a classes.dex-only dynamic top app strip accessibility repair. `AppListAdapter.onBindViewHolder` now binds each dynamic `AppItem` view to a contentDescription starting `smartisax:onestep:app|label=...|package=...|component=...|user=...|id=...`, marks the view important/clickable for accessibility, and attaches `AppListAdapter$AgentAppClickListener`, whose `ACTION_CLICK` route dismisses dialogs and calls the same `AppItem.openUI(context)` used by stock finger clicks. The Sidebar package identity, `android.uid.system` shared UID, `coreApp`, `SidebarService`, and providers are retained; only `classes.dex` changes and the copied v2/v3 signing block remains present with expected digest-boundary status. Smartisax APK hash is `b9eeece7776a35dff470bfae30ec9e6d61ea0e8856ae8df081550a30c1259fea`, Sidebar APK hash is `2ceb4dca8d6e9b2c709cf19064b064d3376e24a12190592ca3cf969cdf6206af`, system_b hash is `59513487018c33383f24c9f163238192f82a596c65d3b89db6ed8a93b0c5ffdc`, and sparse hash is `3f0ea7fb8f3bed0dcf9e8c3582e40c02f0b2db59991ef606a887b8d7cd979f8b`. Build result is `PASS_BUILD_V0AGENT08_ONESTEP_A11Y_NODES`; offline result is `PASS_OFFLINE_IMAGE_V0AGENT08_ONESTEP_A11Y_NODES` with `PASS_AGENT0_OFFLINE_TESTS`, `sidebar_agent_onestep_a11y_nodes=ok`, and `agent08_extra_offline_checks=ok`; flash result is `PASS_FLASH_V0AGENT08_ONESTEP_A11Y_NODES`; read-only result is `PASS_READ_ONLY_V0AGENT08_ONESTEP_A11Y_NODES`. Post-flash proof: boot_completed=1, slot `_b`, bootanim stopped, verified boot orange, root available, SELinux Enforcing, package versionName `0.7.8`/versionCode `59`, device Smartisax APK and Sidebar APK hashes match, Smartisax Shell focused, `isKeyguardShowing=false`, and enabled Accessibility services include `com.smartisax.browser/com.smartisax.browser.SmartisaxAccessibilityService`. A safe One Step enter/exit smoke passes through the programmatic path, and visible One Step windows appear through Accessibility as three roots/two windows, but `uiautomator` does not show the Sidebar app nodes. The Settings task reached `running` with `A11y=100 nodes / 3 roots / 2 windows` and One Step visible, then exposed a runtime diagnosis bug: the Agent worker can die while status remains `running step=0/5`, leaving no visible transcript/action. Evidence: `hard-rom/inspect/v0.agent0.8-onestep-a11y-nodes/build-v0.agent0.8-onestep-a11y-nodes-20260701-185031.txt`, `hard-rom/inspect/v0.agent0.8-onestep-a11y-nodes/verify-v0.agent0.8-onestep-a11y-nodes-offline-image-20260701-190024.txt`, `hard-rom/inspect/v0.agent0.8-onestep-a11y-nodes/flash-v0.agent0.8-onestep-a11y-nodes-20260701-191116.txt`, `hard-rom/inspect/v0.agent0.8-onestep-a11y-nodes/boot-wait-v0.agent0.8-onestep-a11y-nodes-20260701-191116.txt`, `hard-rom/inspect/v0.agent0.8-onestep-a11y-nodes/verify-v0.agent0.8-onestep-a11y-nodes-device-read-only-20260701-191628.txt`, `hard-rom/inspect/v0.agent0.8-onestep-a11y-nodes/one-step-smoke-20260701-191739/report.txt`, `hard-rom/inspect/v0.agent0.8-onestep-a11y-nodes/uiautomator-onestep-20260701-191847/`, and `hard-rom/inspect/v0.agent0.8-onestep-a11y-nodes/settings-task-retry-ui-20260701-192212/effective-run-report.txt`.

v0.agent0.9-worker-a11y-targets: previous live flashed/read-only Agent repair line on top of live/read-only v0.agent0.8. It updates Smartisax to v0.7.9/versionCode 60 and keeps the v0.8 Sidebar app-node APK unchanged. The SmartisaxShell repair catches `Throwable` in the Agent worker loop, reconciles dead workers in `/api/agent/status` and Shell UI from fake `running` to `error/agent_worker_not_alive`, and adds `accessibilityTargets` with `oneStepAppNodeCount`, `settingsNodeCount`, and compact node samples. Smartisax APK hash is `6467308d01a308526f2f5f652372942d709e8257e4b7a40ce194645a7b7e26fc`, Sidebar APK hash remains `2ceb4dca8d6e9b2c709cf19064b064d3376e24a12190592ca3cf969cdf6206af`, system_b hash is `65867365776dbf8d4c73c1ab26a16f8e9d8bf5e47758909b811b698586cf589f`, and sparse hash is `648320622194a61fa0f4c4b9d30f5d395c6f20928e5c53bd98896c4a705a6cfc`. Build result is `PASS_BUILD_V0AGENT09_WORKER_A11Y_TARGETS`; offline result is `PASS_OFFLINE_IMAGE_V0AGENT09_WORKER_A11Y_TARGETS` with `PASS_AGENT0_OFFLINE_TESTS`, `sidebar_agent_onestep_a11y_nodes=ok`, and `agent09_extra_offline_checks=ok`; flash result is `PASS_FLASH_V0AGENT09_WORKER_A11Y_TARGETS`; read-only result is `PASS_READ_ONLY_V0AGENT09_WORKER_A11Y_TARGETS`. Its Settings task is a click-node path PASS and a finish-gate diagnostic: MiMo executed `one_step(enter)` -> `click_node(n7759ef50f9) 100%`, Settings became foreground, and then `finish` paused at `finish_requires_verified_screen_change`. Evidence: `hard-rom/inspect/v0.agent0.9-worker-a11y-targets/settings-task-20260701-195648/report.md`.
v0.agent0.10-finish-target-verify: current live flashed/read-only Agent line on top of live/read-only v0.agent0.9. It updates Smartisax to v0.7.10/versionCode 61 and makes `finish` verification target-aware for Settings-open goals: foreground package `com.android.settings`, a Settings Accessibility window/title, or an actual Accessibility node whose package is `com.android.settings` can satisfy completion. It preserves worker reconciliation, `accessibilityTargets`, `click_node`, the v0.8 Sidebar app-node patch, Portal `/api/agent/status`, and the no-HTTP-input boundary. Smartisax APK hash is `eba484b5ab51ceb08e2afe8413e3bbd6339fde91782540cd4fbc9512687cff37`, Sidebar APK hash remains `2ceb4dca8d6e9b2c709cf19064b064d3376e24a12190592ca3cf969cdf6206af`, system_b hash is `79508a660d2b412fba6eb94582f11aba4947ce4e2fa85e500d33742eb2ad2ca0`, and sparse hash is `66ce7c3013138f05e7789d851ebadd8f5cb686b208084331270b11e82df0d8bc`. Build result is `PASS_BUILD_V0AGENT010_FINISH_TARGET_VERIFY`; offline result is `PASS_OFFLINE_IMAGE_V0AGENT010_FINISH_TARGET_VERIFY` with `PASS_AGENT0_OFFLINE_TESTS`, `sidebar_agent_onestep_a11y_nodes=ok`, and `agent010_extra_offline_checks=ok`; flash result is `PASS_FLASH_V0AGENT010_FINISH_TARGET_VERIFY`; read-only result is `PASS_READ_ONLY_V0AGENT010_FINISH_TARGET_VERIFY`. Post-flash proof: boot_completed=1, slot `_b`, bootanim stopped, verified boot orange, root available, SELinux Enforcing, package versionName `0.7.10`/versionCode `61`, device Smartisax APK hash matches, and system WebRTC libs match. The first post-flash Settings diagnostic paused before any model action with `provider_network_dns_unavailable`; after Wi-Fi/default-network recovery, MiMo reached planning normally. The first network-restored rerun returned `one_step(enter)` but paused at `one_step_enter_not_visible` because Sidebar logged `request zoom failed cause keyguard showing`. After UI normalization (wake/dismiss keyguard, HOME, force One Step exit, relaunch Shell), the same Settings goal completed in 4/5 steps: One Step became visible, Settings was foreground by 10s, `A11y Targets` reported `13 One Step apps / 1 Settings`, transcript included `click_node(ncd691c1cce) 100%` with `accessibility: ok`, and `finish 100%` ended state `complete` once foreground/Accessibility evidence proved Settings was visible. Evidence: `hard-rom/inspect/v0.agent0.10-finish-target-verify/build-v0.agent0.10-finish-target-verify-20260701-203435.txt`, `hard-rom/inspect/v0.agent0.10-finish-target-verify/verify-v0.agent0.10-finish-target-verify-offline-image-20260701-204145.txt`, `hard-rom/inspect/v0.agent0.10-finish-target-verify/preflight-v0.agent0.10-finish-target-verify-20260701-204456.txt`, `hard-rom/inspect/v0.agent0.10-finish-target-verify/flash-v0.agent0.10-finish-target-verify-20260702-140853.txt`, `hard-rom/inspect/v0.agent0.10-finish-target-verify/verify-v0.agent0.10-finish-target-verify-device-read-only-20260702-141359.txt`, `hard-rom/inspect/v0.agent0.10-finish-target-verify/settings-task-20260702-141444/report.md`, and `hard-rom/inspect/v0.agent0.10-finish-target-verify/settings-task-rerun-20260702-142354/report.md`. Next gate: add Shell Agent panel auto-refresh for running transcript/status and add One Step keyguard/readiness preflight before `one_step enter`; Portal WebRTC regression remains a separate follow-up after explicit Portal enable/pairing.
...
recently removed local large build outputs:
  ...
  On 2026-07-01, after the user asked to recover roughly 50 GiB free space,
  all regenerable `hard-rom/work/*` directories, superseded Agent sparse images
  v0.agent0 through v0.agent0.7, superseded Portal sparse images
  v0.portal5y through v0.portal6f, and old raw system_b intermediates were
  removed. Free space rose from about 2.6 GiB to 61 GiB before rebuilding
  v0.agent0.10, and remains about 53 GiB after v0.agent0.10 build/offline
  verification/preflight and live flash. The retained local direct flash targets are now v0.4
  rollback, v0.portal6g, v0.agent0.8, v0.agent0.9, and v0.agent0.10.
